...
**Секвенции кадров** (**superseded ADR-0006**). Оба таргета `meta.target` **не реализованы**: ни side-mask (`alphaextract` + `hstack`), ни атлас-упаковки кадров в конвейере нет, само поле `meta.target` никто не читает. Факт: `vn assets video seq` склеивает кадры в видео-**мастер** (`libx264` CRF 12) в `assets_src/video_src/<group>/<name>.mp4`, дальше работает общий видео-трек («Видео» ниже) — то есть от замысла живёт только «секвенция → webm», и без альфы. Ниже — исходный замысел, а не сегодняшний конвейер:
