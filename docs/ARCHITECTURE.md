...
**G1. Один CLI — `vn`.** Все инструменты — один Python-пакет `tools/vn/`: `vn bootstrap|doctor|dev|build|play|package`, `vn assets …`, `vn content …`, `vn scene …`, `vn chapter …`, `vn char …`, `vn loc …`, `vn voice …`, `vn save …`, `vn test …`, `vn release …`, `vn pack …`. Других CLI не существует; CI-режим проверки без записи — везде флаг `--check`.
...
> **Фактическое состояние (2026-08-24).** Выборочного smoke на MR НЕ СУЩЕСТВУЕТ: флага `vn test smoke --affected` в CLI нет (грепом по `tools/vn/src/vn/` — ноль вхождений), и `.github/workflows/ci.yml` не запускает ни одного прогона игры вовсе. Все `vn test *` живут в `nightly.yml` и `canary.yml`. Значит на каждый пуш игру никто не играет: правка, ломающая прохождение, мержится зелёной и всплывает на следующем nightly — а `release.yml` QA-гейтов тоже не гоняет (только `vn release build`), поэтому тег, поставленный до следующего nightly, увозит дефект игрокам. Что на MR есть сейчас: `vn content lint`, `vn build`, `renpy lint`, `vn loc keys --check`, `vn test oversample`, `vn content compile --check`, `vn content flow`, `pytest`. Решение «реализовать `--affected` или зафиксировать, что прогоны игры только в nightly» не принято; до тех пор читать первый абзац как ПЛАН, а не как факт.

**G16. Каждый YAML начинается с `schema: <name>@<int>`.** Без исключений; реестр схем в `tools/schemas/`. Бамп версии схемы и перевод всех документов на неё едут ОДНИМ PR: реестр держит версии рядом, поэтому отставший документ прошёл бы валидацию молча — его ловит гейт «документ не имеет права быть на устаревшей версии» (`SchemaRegistry.validate`). Отдельной команды миграции нет (ADR-0017).
...
- Трасса выборов прогона: `picks.log` автопилота (`menu N -> pick K (<menu_id>)`) — отдельного якоря в ветке нет (ADR-0019).
...
Плоская модель с префиксом: `persistent.vn_*` (никакого dict-корня `persistent.vn`). Формулировку 6.1 переписать. Разблокировка галереи — штатный механизм `Gallery` + `persistent._seen_images`; `persistent.gallery_unlocked` не существует.

> **Уточнено [ADR-0010](adr/0010-gallery-extras.md):** «свой dict не ведётся» верно только для `kind: image`/`kind: shot` с `unlock: {seen_image: true}` — там кадр засчитывает сам движок. Видео (`Movie`) движок в `_seen_images` не пишет вовсе, а якоря `scene`/`beat`/`var`/`chapter_done`/`always` ему неизвестны, поэтому для них ведётся `persistent.vn_gallery_unlocked` (id → True; имя по этому же C9). Дублирования нет: для картинок с `seen_image` свой набор не используется. Убирать это поле, следуя букве нормы, НЕЛЬЗЯ — потеряются все разблокировки видео и якорей у всех игроков.
...
**C24. Галерея**
Механизм разблокировки — штатный `Gallery` + `persistent._seen_images` для `kind: image`/`shot` с `unlock: {seen_image: true}`; видео и якоря `scene`/`beat`/`var`/`chapter_done`/`always` — через `persistent.vn_gallery_unlocked` (**уточнено [ADR-0010](adr/0010-gallery-extras.md)**, см. врезку к C9). Генерат: `game/generated/screens/gallery.gen.rpy`. Пути превью/CG: `assets/cg/…`, `assets/bg/…`, `assets/shots/<chNN>/<sNNN>/<shot>.thumb.webp` (композитное превью послойного шота, трансформация `shot_thumb`) — сегмента `images/` не существует нигде (в т.ч. в путях loc-оверлеев: зона `assets/loc/`).
...
| сервис | — (`vn shell` и `vn migrate` выведены из нормы, ADR-0017) |
...
- Сохраняемые переменные **никогда не начинаются с `_`** — это НАША конвенция «переменная транзиентна», а не свойство движка: Ren'Py кладёт в сейв и underscore-префиксные переменные стора (`rollback.get_roots` перебирает `ever_been_changed` без фильтра по имени; проверено на фактических сейвах проекта — в корнях лежат `store._vn_ap_shot`, `store._chapters`). Следствие, о котором надо помнить: положить в `_`-переменную объект — значит положить его в каждый сейв.
...
default vn_save_schema = 7       # БЕЗ "_"-префикса: "_" у нас означает «транзиентная»,
...
- **Версии схем данных — сквозные**: **каждый** YAML в репозитории начинается с `schema: <name>@<int>` (имена схем без префиксов: `scene@1`, `chapter@1`, `character@1`, `vars@1`, `asset_src@1`, …) — включая themes, ui, staging, pipeline, loc, flags и манифесты механик, а не только сцены и главы. Реестр JSON Schema в `tools/schemas/` — единственный источник; `vn content lint` фейлит файл без поля `schema` и файл, оставшийся на предыдущей версии. Переход на новую версию — одним PR на весь репозиторий: в нём и бамп схемы, и перевод документов, и (если структура изменилась) правка кода-читателя. Отдельная команда миграции не нужна и не существует: она имела бы смысл при зоопарке версий, а гейт как раз не даёт ему возникнуть (ADR-0017).
...
| Видео | `video_src/(?P<group>[a-z][a-z0-9_]*)(?:/(?P<sub>[a-z][a-z0-9_]*))*/(?P<name>[a-z][a-z0-9_]*)\.(mp4|mov|mkv|webm|m4v|avi)` (группа обязательна: имя без группы — ошибка сборки; вложенные подгруппы допускаются — каждый сегмент проверяется тем же паттерном, а NSFW-мастера живут в группе `nsfw/`, которую SFW-флейворы исключают целиком) | `video_src/demo/ambient.mp4`, `video_src/nsfw/rooftop/kiss.mp4` | `video_src/opening (final2).mov`, `video_src/ambient.mp4` (нет группы) |
...
**Секвенции кадров** (**superseded ADR-0006** — отзыв зафиксирован в шапке ADR-0006). Оба таргета `meta.target` **не реализованы**: ни side-mask (`alphaextract` + `hstack`), ни атлас-упаковки кадров в конвейере нет, само поле `meta.target` никто не читает. Факт: `vn assets video seq` склеивает кадры в видео-**мастер** (`libx264` CRF 12) в `assets_src/video_src/<group>/<name>.mp4`, дальше работает общий видео-трек («Видео» ниже) — то есть от замысла живёт только «секвенция → webm», и без альфы. Ниже — исходный замысел, а не сегодняшний конвейер:
...
**Видео** (реализовано ADR-0006; матрица профилей ниже — **superseded**, см. блок отзыва в ADR-0006). Факт: мастера из `assets_src/video_src/<group>/` (`.mp4/.mov/.mkv/.webm/.m4v/.avi`) транскодируются **одним** прогоном ffmpeg (`-c:v libvpx-vp9 -b:v 0 -crf …`), профилей два — `full` (CRF 30, потолок 1080p) и `draft` (CRF 42, потолок 720p, для итераций); аудио по умолчанию вырезается (`-an`), при `keep_audio: true` — `libopus` 96k **без** `loudnorm` (`tools/vn/src/vn/assets/video.py: encode_args`). Superseded-обещания: VP9 2-pass, профили `hd` CRF 33 / `mobile` CRF 36, Opus 128k с `loudnorm` до −16 LUFS — **не реализованы**: `video_total_mb` не выбран и близко, поэтому экономия размера не окупает второй проход (roadmap, P3). Кодоген — `Movie`-образы из `mov_meta` (`content/images.py`); `renpy.movie_cutscene`-хелперов нет. H.264/MP4 не используется: VP9/WebM — самый предсказуемый декод во всех сборках Ren'Py.

**Аудио** (единого компилятора громкости нет — **superseded ADR-0006**, отзыв зафиксирован там же). Факт: музыка, эмбиенс и SFX кладутся в `assets_src/audio_stems/{bgm,amb,sfx}/` **уже готовыми `.ogg`** и копируются в `game/assets/audio/{bgm,amb,sfx}/` побайтово (трансформация `copy_audio`) — нормализацию делает автор вне репозитория, `loudnorm` в этой ветке не вызывается. `loudnorm` есть только у голоса, и цель там другая: `I=-19` (§4.9/C18, `voice.py: encode_opus`), не −16. В сценах — только логические id: кодоген эмитит `define audio.<id> = "assets/audio/bgm/<file>.ogg"`, сценарист пишет `play music <id>`; сырые пути в play-операторах запрещены гейтом сборки — их отклоняет компилятор сцен (`content/scenes.py: _validate_refs`, вызывается из `validate_scene`), а не `vn content lint`, который `.rpy` не разбирает. Голос — отдельный контур: операторы вида `voice vn.voice_path("<line_id>")` генерируются из voice-манифестов `content/chapters/chNN/voice/<lang>.voice.yaml`, файлы — `.opus`, поставка — языковыми паками; см. разделы 5 и 6.
...
- `vn char sheet mira` (домен `char`, раздел 4) → `build/review/mira/index.html`: сетка допустимых комбинаций поза × наряд × эмоция (собранные композиции в z-порядке layeredimage, не сырые слои) плюс замечания валидатора на той же странице. Это формат арт-ревью: лид смотрит одну страницу вместо тыканья по PSD. Offset-рамок, автора и даты сырца в ячейках нет — их негде взять (ADR-0018).
...
- Для игровой галереи компилятор из `gallery: true` + `tags` генерирует `game/generated/screens/gallery.gen.rpy` (thumbnails 480px рендерятся тем же пайплайном, сетка — из registry; пути превью и CG — `assets/cg/…`). Разблокировка — **штатный механизм `Gallery` + `persistent._seen_images`** для кадров (Ren'Py сам отмечает показанные изображения); для видео и якорей `scene`/`beat`/`var`/`chapter_done` — `persistent.vn_gallery_unlocked` (ADR-0010: движок про эти события ничего не знает). Правило «в сейв и persistent — только str/int/float/bool/list/dict» сквозное, см. раздел 6.
...
- **Версионирование схем**: документы репозитория всегда на СТАРШЕЙ версии схемы (гейт `SchemaRegistry.validate`); предыдущие версии остаются в реестре только для чтения истории — артефактов и сейвов. Переход — одним PR: схема, документы, код (ADR-0017).
...
label ch03_s050:
    ...
    # _vn_exit транзиентна намеренно ("_"-префикс — наша пометка транзиентности): между
...
Маркер `$ vn_menu = "ch03_s050_m001"` вставляется перед menu-стейтментом в авторский источник инструментом `vn loc keys` (однажды, монотонный счётчик на сцену — id меню стабилен при любых последующих вставках). Кастомный `screen choice` по маркеру и индексу пункта берёт переведённый текст через `vn_loc.choice_text(vn_menu, idx, caption)` (раздел 5). Стабильная точка для автопилота и аналитики — не якорь в ветке, а трасса прогона: `picks.log` пишет `menu N -> pick K (<menu_id>)`, и по ней считается покрытие ветвления (`vn test paths`, ADR-0019). `default vn_menu = None` объявлен во framework — переменная без `_`-префикса, т.е. честно сохраняется: загрузка сейва, сделанного на открытом меню, восстанавливает lookup.

Галерея опирается на штатный механизм `Gallery` + `unlock_image` (CG засчитывается самим фактом `show cg_ch03_market_rain` в сцене через `persistent._seen_images` — ручного кода разблокировки для КАДРОВ нет; видео и якоря ведутся в `persistent.vn_gallery_unlocked`, ADR-0010), тоже циклом по реестру:
...
# "_" у нас помечает транзиентные переменные, а этой ехать в сейв обязательно.
...
Валидатор (`vn char validate` — быстрая проверка одного персонажа БЕЗ сборки ассетов; гейт на каждый прогон CI исполняет `vn build` тем же кодом, ADR-0018) проверяет в обе стороны:
...
Отчёт по всем персонажам — `vn char validate --all` (строка на персонажа: сколько поз/нарядов/эмоций объявлено, сколько слоёв собрано, сколько поз реально эмитируется). Отдельного HTML-отчёта у валидатора нет: HTML-артефакт один — лист ревью, и блок замечаний печатается в нём (ADR-0018).
...
**Измеренная часть (2026-08-18).** Конвейер сборки на масштабе проверен прогоном, а не рассуждением (`vn test corpus`, 7.6): 20 000 сцен и 10 000 мастеров образов проходят весь путь `assets build → lint → compile → повторный compile → модель памяти` без ошибок; генерат линеен (3,1–3,6 КБ `.rpy` на сцену), повторная компиляция переписывает ноль файлов. Сверхлинейной была ровно одна стадия — **модель памяти образов**: стоимость персонажа считалась на каждого участника каждой сцены, то есть время росло как произведение «сцены × выходы» (0,94 → 8,30 с при росте сцен в 10 раз). Предындексация сделана — `assets/memory.py: _character_index` проходит манифест ОДИН раз до цикла по сценам, дальше стоимость берётся lookup'ом; приведённые числа сняты ДО неё и требуют повторного замера (`vn test corpus`). Прочее ниже — по-прежнему про **рантайм** движка, а он корпусом не измерен.
...
**Контракт наружу.** Другие подсистемы (сцены, галерея, локализация, чит-меню) не читают папки персонажей напрямую — только Character Registry: `define VN_CHARACTERS` в рантайме и JSON-реестр для инструментов. Для каждого ключа там лежат допустимые атрибуты, дефолты, voice tag, слой (`config.tag_layer` генерируется отсюда же) и теги галереи. Точка сопряжения версионируется схемой (`schema: character@1`); переход на новую версию — одним PR: схема, декларации, код-читатель (гейт версий не даёт документу отстать, см. раздел 3 и ADR-0017). Метрика готовности тулинга для этой подсистемы — из общего онбординг-контракта (раздел 1): новый художник от чистой машины до «слой виден в игре» — меньше одного дня.
...
Имя генерата — только по id, без слуга (`ch01_s003.gen.rpy`): statement-имена Ren'Py включают имя файла, слуг в имени генерата ломал бы save-совместимость при переименовании. У пунктов menu нет никаких id-клауз — их не существует в языке. Якоря ветки в генерате нет: покрытие и повтор строятся по трассе прогона (`picks.log`), а не по инжектированным вызовам (ADR-0019). С явными id у say вся связка «перевод + seen-tracking + именование озвучки + скриншоты для переводчиков» держится на одном стабильном ключе.
...
- `default vn_menu = None` объявлен во framework. Имя — **без underscore-префикса**: `_` у нас помечает транзиентные переменные (см. раздел 6), а `vn_menu` обязана корректно сохраняться и откатываться — сейв посреди меню и rollback к меню переисполняют `screen choice`, и id должен быть актуальным. Присваивание `$ vn_menu = ...` стоит в исходнике непосредственно перед menu, поэтому rollback-лог восстанавливает его автоматически.
...
- Транзиентные переменные, не влияющие на прохождение (позиция скроллбара и т.п.), объявляются **с префиксом `_`**. Это НАША пометка «не состояние прохождения», а не механизм движка: правила «Ren'Py не кладёт `_`-переменные в сейв» не существует — `rollback.get_roots()` перебирает `ever_been_changed` всех store_dict'ов без фильтра по имени (единственный фильтр по `_` в SDK — `loadsave.py`, и он про имена СЛОТОВ). Проверено на фактических сейвах проекта: в корнях лежат `store._vn_ap_shot`, `store._vn_ap_menu`, `store._chapters`. Практическое следствие: `_`-переменная — не свободная зона; положить в неё экземпляр класса значит положить его в каждый сейв, и переименование класса в следующем релизе сделает сейвы игроков нечитаемыми. Требование «в сейве только простые типы» относится к ним ровно так же, как к остальным.

**`renpy.persistent`** (глобальные анлоки, настройки) — отдельный контур с теми же правилами: только простые типы (в том числе никаких `set()` — вместо него dict-множество `{id: True}`), **плоские переменные с префиксом `persistent.vn_*`** (никакого dict-корня), декларации — тот же формат `vars@1` со `store: persistent` (компилятор эмитит `default persistent.vn_<var>`) и свои редкие миграции. Ren'Py мержит persistent между инсталляциями пофилдово, и по умолчанию побеждает более новое значение поля ЦЕЛИКОМ (`persistent.py: default_merge`). Плоскость имён сама по себе предсказуемого слияния НЕ даёт: поле-накопитель (`{id: True}`) при слиянии двух инсталляций теряет записи одной из сторон независимо от того, лежит оно в корне или в общем dict. Предсказуемым слияние делает `renpy.register_persistent(field, merge)` — так движок поступает со своими множествами (`_seen_images`, `_seen_audio`, `_chosen`), и так же обязаны быть зарегистрированы наши накопители: таблица полей и функции слияния — `engine_compat/000_compat.rpy: PERSISTENT_MERGES`, покрытие проверяет обнаруживающий тест. Кастомные классы в persistent непредсказуемы вовсе. Разблокировка галереи для кадров собственного учёта не ведёт: используется штатный механизм `Gallery` + `persistent._seen_images`. Для видео и якорей ведётся накопитель `persistent.vn_gallery_unlocked` — он же зарегистрирован в `PERSISTENT_MERGES` (ADR-0010; экран — см. раздел 7).

### 6.2. Версия схемы и метаданные сейва

Версия схемы состояния — **одно монотонно растущее целое** в переменной `vn_save_schema`. Имя — сознательно **без underscore-префикса**: `_` у нас помечает транзиентные переменные, то есть вариант с underscore-префиксом из ранних черновиков никогда не увидел бы старую версию схемы при загрузке. Источник числа — `project.yaml` (`save_schema: 12`, см. 6.6), объявление генерируется в `defaults.gen.rpy`.
...
vn test smoke [--picks] [--lang] [--record <имя> --why <зачем>]
vn test screens [--variant]                        # тур по content/ui/screens.yaml
vn test paths [--picks …] [--strict]               # покрытие сцен и рёбер выбора
vn test replay [<имя>|--all]                       # повтор записанного прогона
...
Каждый YAML в репозитории начинается с `schema: <name>@<int>`; `vn content lint` фейлит файл без поля schema и файл на устаревшей версии — это сквозное правило, включая темы и ui-декларации этого раздела.
...
schemas:      { stage: validate, script: [vn content lint] }
...
smoke:
  stage: test
  needs: [compile]
  script: [xvfb-run -s "-screen 0 1920x1080x24" vn test smoke --picks 0,0   # ВНИМАНИЕ: флага --affected не существует, см. врезку к G15]
...
package:
  ...
  script:
    ...
    - vn release android build --bundle                      # вес пакета против потолка канала по ФАКТУ файла (7.7)
...
nightly-perf:
  ...
  script: [vn test smoke --picks 0,0]                        # cold start, пик RSS и вес .rpyc против бюджетов (ADR-0019)
```
...
- меню — обычные `menu`-стейтменты. Клаузы `id` у пунктов menu в Ren'Py не существует — идентичность меню несёт переменная `vn_menu` (формат id — `m\d{3}`): присваивание `$ vn_menu = "ch02_s030_m001"` вставляет инструмент `vn loc keys` в авторский scene.rpy перед каждым menu-стейтментом; id фиксируется в menu-реестре и в PO (msgctxt), см. раздел 5. `default vn_menu = None` объявлен во framework — имя намеренно без underscore-префикса, чтобы значение попадало в сейв. Якоря в ветке нет: точка привязки для повтора и аналитики — трасса прогона `picks.log` (`menu N -> pick K (<menu_id>)`), ADR-0019;
...
- Запись: `vn test smoke --record <имя> --why <зачем>` → `ci/fixtures/replays/<имя>.vnrec.json` (`replay@1`): индексы выборов, язык и вариант окружения плюс ожидаемый результат (вердикт, трасса, состояние, галерея). Seed не пишется — RNG в игре нет; ввод не пишется — свободного ввода нет (ADR-0016, ADR-0019).
...
**Генерат линеен: 3,1–3,6 КБ `.rpy` на сцену на всех масштабах**, повторная компиляция переписывает ноль файлов (идемпотентность проверяется внутри прогона, а не наблюдается). Долю бюджета из таблицы нельзя переносить на боевое дерево напрямую: в `game/generated` корпуса нет `.rpyc` — движок по нему не запускается, — а в бою пара `.rpy`+`.rpyc` весит ×2,5 (замер по сценам репозитория), то есть 64 МБ вмещают около 7,5 тыс. сцен на 8 реплик и около 7 тыс. боевых сцен на 30 реплик. И то и другое — сотни глав. Сверхлинейной была модель памяти: стоимость персонажа считалась на каждого участника каждой сцены, поэтому время зависело от произведения «сцены × выходы» — при фиксированных 2 000 сценах рост выходов в 16 раз давал ×2,9, при фиксированных выходах рост сцен в 10 раз — ×8,8 (0,94 → 8,30 с). Предындексация сделана: `assets/memory.py: _character_index` строит `{персонаж: худшая поза}` одним проходом до цикла по сценам (обоснование — в его докстринге). Числа выше сняты ДО этой правки; чтобы цитировать их дальше, повторите прогон `vn test corpus`.
...
Суммарный размер `.rpyc` лимитируют бюджеты `vn build` (`budgets.generated_total_kb`) — компактность кодогена там, где развёртка не нужна статическому анализу, обязательна (пример — экраны глав в 7.8; predict-списки эмитятся данными, а не построчными вызовами).
...
В режиме `screens` автопилот открывает каждый экран из декларации `content/ui/screens.yaml` (`qa_screens@1`), ждёт кадр и вызывает `renpy.screenshot()`. Гейт **структурный, не пиксельный** (ADR-0019): провал — экран не открылся, экрана нет в игре, кадр не появился или в игре есть экран, не названный ни в туре, ни в `ignore_defined`. Пиксельные эталоны отклонены: они привязаны к окружению съёмки (рендер шрифтов и llvmpipe у раннера и у разработчика различаются), своего CI-образа у проекта нет (ADR-0017), и до устоявшегося UI их сопровождение дороже пользы.
...
- Счётчик схемы сейва в build_info **не дублируется**: единственный счётчик — `default vn_save_schema = N`, генерируемый в `game/generated/state/defaults.gen.rpy` (раздел 6). Имя намеренно без underscore-префикса: `_` в проекте помечает транзиентные переменные, а счётчику схемы ехать в сейв обязательно. Значение дублируется в `config.save_json_callbacks`, чтобы `vn save check` читал версию без unpickle.
...
- События: `scene_enter/exit`, `choice`, `ending_reached`, `crash`. Точки съёма — те же, что у QA: `vn.checkpoint` (сцены) и трасса выборов прогона; сценаристы о телеметрии не знают.
...
Каждая сборка публикует артефактом `build_report.json`: граф задач, cache hit/miss, тайминги (включая суммарную длительность MR-пайплайна против бюджета < 10 минут), размеры финальных артефактов по платформам (.aab, universal .apk, десктопные архивы) и дельта к прошлому релизу. Бюджеты на размер и время проверяются `vn build` (ассеты и генерат) и `vn release android build` (вес мобильного пакета по факту файла).
...
|---|---|
| 1 — вертикальный срез (мес. 1–3) | `vn.buildcore`, `vn build`/`vn content compile`, `vn assets build` (PNG/PSD/аудио), `vn content lint`, `vn bootstrap` + bootstrap-check, базовый CI (schemas → compile → lint → smoke затронутых глав). Ничего больше |
...
| Версия | `^\d+\.\d+\.\d+(\+[0-9a-z.]+)?$` |
| Кадры тура экранов | `.vncache/screens/screen_<имя>.png` — производный артефакт прогона (пиксельных эталонов нет, ADR-0019) |
...
- Документы всегда на старшей версии схемы: бамп и перевод контента едут одним PR, отставший документ валит гейт версий (ADR-0017).
