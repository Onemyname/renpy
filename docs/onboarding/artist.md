...
- **The Sims 4** — опциональный четвёртый источник **за лицензионным гейтом EA**
  (ADR-0007). Технически — как VaM: `assets_src/sims4/**/<имя>.render.yaml`
  (schema `sims4_render@1`), захват в `png/cg/**` / `video_src/**`,
  `vn assets sims4 validate`; установка — `tools/install-sims4.ps1`.
  Два жёстких правила: в декларации **обязательна `game_version`** (патчи EA
  меняют картинку) — и пока лицензия с EA не урегулирована, Sims4-контент
  **не пройдёт релизный гейт** (готовить можно, отгружать нельзя).
