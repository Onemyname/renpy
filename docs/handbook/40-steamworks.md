...
> **⚠️ Настроено на плейсхолдерах (2026-08-21):** App ID 480 / депоты 481–483 /
> DLC 484–485 — тестовые номера, конвейер прогнан целиком (VDF, раскладка
> депотов, preflight «0 шагов»). При заведении реального приложения — заменить
> номера в project.yaml и packs/*/manifest.yaml, детали в ci/steam/README.md.
>
> **Статус подсистемы:** PARTIALLY IMPLEMENTED — техническая часть проекта готова (`define config.steam_appid` из генерата, ачивки и DLC-владение через штатный стек движка), и **раскладка депотов работает на всех трёх платформах**: `steam_stage_content` знает фактические форматы пакетов движка — zip для Windows/mac и `tar.bz2` для Linux (§4.3), — и ожидает только те платформы, у которых объявлен депот. Но **поставка целиком не пройдена**: **приложения в Steamworks ещё нет** — в `project.yaml` стоят плейсхолдеры Valve (`appid: 480` — Spacewar, депоты 481–483, DLC 484–485 в манифестах паков), на них конвейер прогнан целиком, но заливка в чужое приложение невозможна (§4.2, `../decisions/owner-decisions.md` §2); steam_api-редистрибутивов Valve на build-машине нет; аплоад — ручной `steamcmd`, в CI не заведён; на живом Steam не проверялось ничего ([43](43-steam-qa.md)). Steam Cloud — **NOT IMPLEMENTED** и это решение (§7).
...
| 3 | Создание приложения | **App ID** (одно число) | `project.yaml: platform.steam.appid` | **нет** — стоит **плейсхолдер** (`480` — Spacewar Valve) |
| 4 | Создание **депотов** контента (обычно по одному на ОС) | **Depot ID** — как правило `appid+1`, `appid+2`, … | `project.yaml: platform.steam.depots.{windows,linux,mac}` | **нет** — стоят плейсхолдеры 481–483 |
...
| 9 | **DLC** как отдельные приложения (для паков) | DLC App ID | `packs/<id>/manifest.yaml: steam_dlc_appid` | **нет** — стоят плейсхолдеры 484 (`ep_beach`) и 485 (`nsfw`) |
...
|---|---|---|
| 1. Прочитать `platform.steam` | `steam_config` (`release.py:215-222`) | нет `appid` → **exit 1**. Сегодня шаг проходит: в `project.yaml` стоит плейсхолдер 480, и прогон доходит до раскладки депотов |
...
**Автоматической проверки выдачи ачивок нет.** Автопилот дампит состояние (`state.json`) и галерею (`gallery.json`, `030_flow.rpy:186-211`), но `persistent.vn_achievements` — нет. Косвенный признак в прогоне: `vn_ach.grant` возвращает `True`, а игрок видит тост — `_ach_notify` (`030_flow.rpy`) шлёт `renpy.notify` со списком локализованных названий на всех трёх штатных путях выдачи (checkpoint, beat, chapter_done). Прямой `vn_ach.grant` из авторского кода при этом молчит: уведомление живёт в обвязке потока, а не в сторе — стор не знает, батч это или одиночная выдача.
