...
Граница автоматизации в этом проекте проходит ровно по `assets_src/`. Слева от неё — человек с мышкой и GPU. Справа — 20 команд `vn`, 7 версионированных трансформаций, 36 схем, 34 правила линта, 19-чековый релизный гейт и 7 определений CI-джоб в 4 workflow. Этот файл — карта обеих сторон и приоритизированный список того, что стоит перетащить слева направо. Критерий приоритета один: **ускоряет ли это производство контента**.
...
| Процесс | Команда / триггер | Что делает | Статус |
|---|---|---|---|
| Юнит-тесты тулинга | `python -m pytest tools/vn/tests -q` | 152 теста в 19 файлах. На машине без `RENPY_SDK` и ffmpeg 11 из них скипаются | IMPLEMENTED — но CLI покрыт почти никак: из `cli.py` (1643 строки) тестами закрыта одна команда `pack build` (`test_release.py:141-192`, через `CliRunner`); `analyze.py`, `scaffold.py`, `psd.py`, `devloop.py` не импортирует ни один тест |
...
- **Не считать `docs/ARCHITECTURE.md` описанием построенного.** `--use-artifact` (14 упоминаний), `vn validate`, `vn test perf`, `rpyc-compat`, каналы dev/beta/release, депоты Steam, `.rpa`-архивы — всё NOT IMPLEMENTED.
