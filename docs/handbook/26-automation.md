...
- **Пиннованный тулчейн (G17).** Во всех строках установки идёт `pip install --quiet -r tools/vn.lock` **перед** `pip install -e "tools/vn[dev]"` — `ci.yml` (2), `nightly.yml` (3, включая новую джобу `corpus`), `canary.yml`, `release.yml`, `steam-upload.yml`, `.gitlab-ci.yml` (2). По числу джоб мест **одиннадцать**: 8 GitHub + 3 GitLab (шаблон `.with-sdk` разворачивается в `build` и `test`) — эту одиннадцатку и ассертит тест. Порядок — часть контракта: поставь editable первым, и пины станут декоративными.
...
**Current state:** PARTIAL / STALE. `.gitlab-ci.yml` — 53 строки, 3 джобы (`lint`, `build`, `test`). Нет: релиза, флейворов, LFS (а именно потеря LFS дала однажды битую сборку со шрифтами-указателями), ffmpeg (первый же видео-сырец делает `vn build` красным), `vn loc keys --check`, smoke, сейв-корпуса, canary. GitHub при этом — 5 workflow, 10 определений джоб. `ci/README.md:6` до сих пор называет GitLab «конфигом пайплайна». `CODEOWNERS` покрывает `/.gitlab-ci.yml` и **не покрывает** `/.github/`.
...
| **Частые ошибки** | 1) Считать `docs/ARCHITECTURE.md` описанием построенного: `--use-artifact`, `vn validate`, `vn test perf`, `rpyc-compat`, депоты Steam — NOT IMPLEMENTED (а `.rpa` — не долг, а норма §2.4: россыпь). 2) Править `.gitlab-ci.yml`, думая, что это пайплайн проекта: настоящий — `.github/workflows/`, 5 workflow, 10 определений джоб (и именно в `.gitlab-ci.yml:60` остался прогон pytest из корня, который красен — [27 §2.1](27-testing.md)). 3) Автоматизировать рендер/генерацию раньше, чем появится первая `*.render.yaml` и первый workflow-JSON — сейчас в репозитории ноль тех и других. 4) Писать свой парсер `.rpy` вместо моста SDK (G24). 5) Ожидать, что `vn content graph` и `vn release changelog` видят главы паков — оба читают только `content/chapters/` (`graph.py:15`, `release.py:124`). 6) Считать `tools/vn.lock` неработающим — он ставится первым во всех 11 местах установки тулчейна (8 джоб GitHub + 3 GitLab, где шаблон `.with-sdk` разворачивается в `build` и `test`), и новая зависимость без пина в нём приедет случайной версией. 7) Добавить джобу CI без `-r tools/vn.lock` до editable или без `ffmpeg` до `vn build` — покраснеет `tools/vn/tests/test_ci_config.py`. 8) Писать в `game/build_id.json` сам patron-токен — документ уезжает игроку целиком; наружу идёт только `patron_tag` (ADR-0011) |
