...
| `.gitignore`, `.gitattributes`, `CODEOWNERS`, `README.md`, `project.yaml`, `.vnstorage.yaml` | `:19-20` | конфигурация процесса, не игры |
...
**Как устроено сейчас.** `vn release build --patron-token <токен>` (`cli.py`) по-прежнему принимает токен, но наружу уходит односторонняя производная:
...
`game/build_id.json` живёт только на время `distribute`: он в `.gitignore:8` и удаляется в `finally` (`cli.py`), поэтому dev-чекаут никогда не носит чужой флейвор и вотермарку.
...
**Питон-зависимости — PARTIALLY IMPLEMENTED, но уже не декоративно.** `tools/vn.lock` (18 закреплённых пакетов) **теперь читается всеми пайплайнами**: `pip install --quiet -r tools/vn.lock` стоит **перед** editable-установкой во всех 8 джобах установки тулчейна (`ci.yml` ×2, `nightly.yml` ×3, `canary.yml`, `release.yml`, `steam-upload.yml`). Порядок — предмет теста `tools/vn/tests/test_ci_config.py::test_lock_installed_before_editable`: если editable окажется первым, pip отрезолвит `>=`-диапазоны и пины станут бесполезными.
...
**Грабля.** `vn pipeline models --only <id>` **скачивает**, а не показывает статус: условие в `cli.py` — `if pull or only_set`. Можно случайно вытянуть `restricted`/`unknown`-модель (bigASP, Civitai-LoRA) в момент, когда решение по ADR-0008 ещё не принято.
...
Реализация — `licenses.py:53-109`, CLI — `cli.py`.
...
| **Chatterbox** (TTS) | MIT, https://github.com/resemble-ai/chatterbox | Самая чистая лицензия в категории. Все выходы несут нейро-вотермарку Perth — это нормально, снимать её не нужно и не следует | кандидат под `vn voice tts` (сама команда — стаб фазы 2, `cli.py`; остальной контур `vn voice` работает — [23-audio.md](23-audio.md) §8) |
...
Файл — единственное, что едет игроку в качестве уведомлений о сторонних компонентах. Механика (`cli.py`): `vn release build` копирует его в `game/THIRD-PARTY-NOTICES.md` **на время** `distribute` и удаляет в `finally` (`cli.py`), чтобы dev-чекаут не носил копию. Разделение зон в самом файле правильное: `content/licenses.yaml` — учёт покупных ассетов (в git, с гейтом), а `THIRD-PARTY-NOTICES.md` — «только то, что уезжает игроку».
...
- **Не запускать `vn pipeline models --only <id>`, думая, что это «показать статус».** Флаг `--only` включает загрузку (`cli.py`) — можно случайно вытянуть `restricted`/`unknown`-модель.
