...
| Завести NSFW-фон | категории `nsfw/` для `bg` в коде нет: `release.py:191-202` считает исключения по **реально существующим** каталогам, а `game/assets/bg/nsfw/` не существует. NSFW сегодня живёт в `cg/` и `mov/`. См. [30-packs-and-dlc.md](30-packs-and-dlc.md) |
