...
**Официальная документация:** сам код — `../../tools/vn/src/vn/cli.py` (2117 строк) плюс [Свой движок](25-custom-engine.md).
...
**Версия (проверено):** **3.12.10** (системная). `pyproject.toml` требует `>=3.10`; `vn doctor` проверяет именно `>= 3.10` (`doctor.py:72-73`). В CI Python ставится `actions/setup-python` (`.github/workflows/ci.yml`).

**Установка:** отдельно от SDK. Проверка — `python --version` и `vn doctor`.

**Конфигурация:** пиннованный тулчейн — `../../tools/vn.lock` (18 пакетов, точные версии). **IMPLEMENTED с 2026-08-08:** лок читается — `pip install --quiet -r tools/vn.lock` стоит перед editable-установкой во всех 8 джобах установки тулчейна — `ci.yml` ×2, `nightly.yml` ×3, `canary.yml`, `release.yml`, `steam-upload.yml`. Порядок «лок раньше editable» не косметика — editable следом уже ничего не поднимает, потому что его `>=`-диапазоны удовлетворены; именно порядок стережёт `tools/vn/tests/test_ci_config.py`. Норма G17 («пиннованный тулчейн, откат = `git revert` этого файла») выполнена для 18 прямых пакетов, и шаг из `../runbooks/pipeline-broken-at-night.md` стал исполнимым. **Остаток:** транзитивные зависимости в локе не закреплены (`pygments`) — одинаковый лок не означает побайтово одинаковое дерево пакетов.
...
| `pytest` | 9.1.1 | 9.1.1 | 373 теста в 27 файлах `tools/vn/tests/` | IMPLEMENTED |
...
- https://adr.github.io/ — ADR как самый дешёвый машиночитаемый ответ на «почему это так и где оно лежит». У нас 16 ADR, см. часть 4. **Устаревший ADR активно вредит:** агент ему поверит и построит против системы, которую вы уже удалили.
...
|---|---|---|---|
| `../ARCHITECTURE.md` | 4182 строки | **нормативный целевой** документ; раздел 0 (G1–G24, C1–C24) — контракт | когда нужно понять *как должно быть*; **никогда** как описание построенного |
| `../adr/` | 16 решений + шаблон | принятые архитектурные решения с контекстом и последствиями | когда «почему это так?» |
| `../conventions/naming.md` | 35 строк | нормативные паттерны id, проверяются линтом | перед созданием любой сущности |
| `../conventions/folder-layout.md` | 42 строки | нормативная структура каталогов | перед созданием каталога |
...
| `../../ci/README.md` | 12 строк | что делает CI | ✅ актуален с 2026-08-18: указывает на `.github/workflows/` (5 workflow) и объясняет, почему второго конфига нет |
...
| `docs/handbook/` (этот каталог) | 46 файлов: карта `README.md` + 45 нумерованных how-to | практический хендбук: «что мне конкретно сделать» | всегда, кроме случаев выше |
...
Ни один ADR не имеет статуса «заменено». Из 16 ADR пятнадцать приняты, один (0008) предложен и ждёт решения владельца. Уточнения существующих решений оформляются врезкой «Уточнено» внутри самого ADR (так закрыт отложенный пункт ADR-0013 про галерею и ADR-0012 про волосяные линии), а не новым документом — новый ADR завёл бы второй источник истины по одному вопросу.
...
| 0012 | Render-профиль, оверсэмпл-варианты и модель памяти образов | единственное место решения «в каком разрешении и формате живут ассеты» — `project.yaml: render`; зона мастеров переименована в `assets_src/art/`, параметры трансформации входят в ключ кэша, worst-case сцены гейтится бюджетом памяти | принято |
| 0013 | Послойные полнокадровые шоты (`shots@1`) | кинематографический кадр = env-подложка + вырезанные слои персонажей; наряд меняется переменной гардероба без перерендера кадра, эмиттер `layeredimage` переиспользован | принято |
| 0014 | Platform Services — Steam как одна из платформ, не фундамент | ядро самодостаточно, платформа — провайдер; единственная точка касания `00_core/035_platform.rpy` под гард-тестом, ownership-провайдер G9 подключён, `vn release steam` легализована (C13) | принято |
| 0015 | Сетка 1920×1080 остаётся; физическое разрешение даёт движок | исходная гипотеза «на Deck текст принципиально мягче» **опровергнута прогоном**: Ren'Py растеризует текст (и SVG) в drawable-разрешении по умолчанию, в обе стороны. Поднятие сетки до 2560/3840 отклонено по счёту (`@2` стал бы 5120×2880 при мастерах 4K → 4K деградирует), «UI-слой в физическом разрешении» отклонён — механизма нет. Вместо этого: `gui.ui_scale`, нижняя граница толщины линий панелей и контракт-тест на два недокументированных `config.*` (G18) | принято |
| 0016 | Свободного текстового ввода в игре нет | решение принято не по платформе, а по G8: введённая игроком строка — непереводимый фрагмент в переводимой реплике, в русском ещё и без склонения. Имя ГГ, если появится, — выбор из закрытого списка (переменная реестра G5). Своя OSK отклонена, движковая `_touch_keyboard` тоже (существует и включается `config.touch_keyboard`, но ASCII-only). `screen input` остаётся как контракт движка; Ren'Py Sync в UI не выводится | принято |
...
Речь о документации **вне** `docs/handbook/`: практические how-to за его пределами существуют ровно для трёх областей — production-окружение, аварийный пайплайн, дизайн экрана выбора. Всё остальное описано только нормативно. Крупнейшие разрывы «код есть — документа нет»:
...
| **Зависимости (что ломается ниже по течению)** | Версии в этом файле должны совпадать с выводом `vn doctor` / `vn pipeline doctor` — иначе агент, читающий хендбук, будет строить планы на несуществующем окружении. Пин `project.yaml:5` (`renpy_sdk`) связан с `doctor.py:133-135` и `.github/workflows/*.yml`; смена пина в одном месте без остальных даёт красный CI. Манифест `tools/comfyui-models.yaml` валидируется схемой `comfyui_models@1` — новое поле требует правки схемы |
