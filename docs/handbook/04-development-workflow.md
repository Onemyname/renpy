...
- `ci.yml` триггерится на `push` в **любую ветку** (`branches: ['**']`, `ci.yml:16-18`) плюс ручной `workflow_dispatch`. Раньше стояло `branches: [main]`, и работа в feature-ветке ехала вообще без проверок — первый прогон случался уже после слияния. `branches: ['**']` матчит ветки и **не** матчит теги: на теге `v*` работает `release.yml`, дублировать его не нужно. `pull_request` убран сознательно — голова PR это тот же push в ветку этого репозитория, и она уже покрыта; вернуть его (ради форков) можно только вместе с гардом `head.repo.full_name != github.repository`, иначе каждый PR будет прогоняться дважды. Инварианты держит `tools/vn/tests/test_ci_config.py`.
