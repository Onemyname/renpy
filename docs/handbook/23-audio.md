...
> **Статус подсистемы:** PARTIALLY IMPLEMENTED. Декларации, кодоген, ветка сборки `copy_audio`, канал `ambient`, дакинг под голос и **весь голосовой контур** (`voice@1`, `vn voice manifest|import|tts|validate`, `vn.voice_path`, транскод `voice_opus` с loudnorm) работают — домен `vn voice` с 2026-08-18 без заглушек: `vn voice tts` синтезирует черновые дубли (§8.1). Но музыки/SFX в репозитории **ноль** (`content/audio/*.yaml` пусты, `tracks: {}`), loudnorm для bgm/amb/sfx NOT IMPLEMENTED (`copy_audio` копирует байты).
...
| Дакинг под голос (`config.emphasize_audio_*`: канал voice приглушает остальные до 0.6) | IMPLEMENTED (штатный механизм движка) | `045_audio.rpy`: `config.emphasize_audio_*` |
...
- **Клауза `if_changed` эмитится** (с 2026-08-23): обвязка каждой сцены исполняет `play` при каждом входе, и без неё один и тот же трек в соседних сценах перезапускался бы с начала — с `fadeout`/`fadein`, то есть слышимым провалом на каждом переходе. Громкость при этом остаётся прежней, и это безопасно: `volume` объявлен у ТРЕКА в `content/audio/*.yaml`, а не у сцены. Если volume когда-нибудь станет пер-сценовым, клаузу придётся снимать — она сохранит громкость первой сцены. **`stop music` по-прежнему не эмитится**: сцена без поля `music:` НЕ глушит музыку — тема продолжает играть, и это осознанное поведение жанра, а не пропуск. Нужна тишина — заведите её явным треком или отдельным решением (тогда это ADR, а не правка эмиттера).
...
Каналы и микшеры: штатные `music` / `sound` / `voice` плюс наш канал `ambient` (`045_audio.rpy:13` — `register_channel("ambient", mixer="music", loop=True, tight=True)`: громкость эмбиенса регулируется слайдером музыки, `tight` даёт бесшовный кроссфейд при смене файла). Дакинг под голос — штатный `config.emphasize_audio_*` (`045_audio.rpy`): пока звучит канал `voice`, остальные каналы приглушаются до 0.6 линейной громкости за 0.5 с; без озвучки конфиг безвреден. Одних `config.*` для этого НЕ хватает: механизм гейтится игровой настройкой `preferences.emphasize_audio`, у которой значение по умолчанию `False` (`renpy/audio/audio.py`: `if not renpy.game.preferences.emphasize_audio: emphasized = False`). Поэтому рядом стоит `config.default_emphasize_audio = True` (применяется на первом запуске), а игрок может выключить приглушение тумблером «Приглушать музыку под голос» в настройках. 0.6 — ЛИНЕЙНАЯ доля, а не позиция слайдера: значение уходит прямо в `set_secondary_volume`. Три слайдера в настройках (`core_screens.rpy:289-293`, строки `ui.prefs.volume{,_music,_sound,_voice}` в `content/ui/strings.yaml`) — слайдер «Голос» управляет микшером озвучки (§8).
...
vn voice tts ch01 [--char mira] [--lang ru]       # TTS-черновики непокрытых реплик (§8.1)
           [--backend piper|say] [--voice NAME] [--rate 1.0]
           [--regenerate-drafts] [--allow-download]
```
...
**Что осталось NOT IMPLEMENTED:** поставка `voice/<lang>/` отдельными voice-паками/Steam-депотами (сегодня opus-файлы едут в основном дистрибутиве; `vn pack build` ассеты не пакует — [30-packs-and-dlc.md](30-packs-and-dlc.md)). Рантайм к пакам уже готов: отсутствующий файл — no-op.

### 8.1 `vn voice tts` — черновая озвучка до записи актёров (IMPLEMENTED 2026-08-18)

Зачем: голосовой контур (voice-операторы, деградация языков, дакинг музыки) проверяется только там, где звук физически есть, а актёров записывают месяцами. `vn voice tts chNN` синтезирует **непокрытые** реплики главы, кладёт их в общую мастер-зону со статусом `draft` — и глава становится озвученной и играбельной сегодня.

```bash
vn voice tts ch01                       # непокрытые реплики исходного языка, первый доступный бэкенд
vn voice tts ch01 --char mira           # только реплики одного персонажа
vn voice tts ch01 --lang en             # дубляж: текст берётся из PO, а не из исходной реплики
vn voice tts ch01 --backend piper --voice ru_RU-irina-medium --rate 1.1
vn voice tts ch01 --regenerate-drafts   # перезаписать существующие черновики (final не тронет)
vn assets build                         # транскод voice_opus -> game/assets/voice/<lang>/<chNN>/
```

**Бэкенды — данные, а не ветки по платформе** (`voice.py: TTS_BACKENDS`; порядок в реестре = приоритет автовыбора):

| Бэкенд | Роль | Голос | Где ищется бинарь |
|---|---|---|---|
| `piper` | **основной**: кроссплатформенный, голоса моделями, звучит как черновик для вычитки | имя модели (`ru_RU-irina-medium`, `en_US-lessac-medium` — дефолты по языку) либо прямой путь к `.onnx`; поиск в `VN_PIPER_VOICES` → `.vncache/piper-voices` → `~/.local/share/piper-voices` | PATH или `VN_PIPER` |
| `say` | **дев-фолбэк, только macOS**: ничего не устанавливать | таблица предпочтений (`ru: Milena`, `en: Samantha`), затем автовыбор по локали из `say -v '?'` | PATH или `VN_SAY` |

Ни одного нет — команда падает с рецептом установки для каждого. Диалект флагов piper (`--output-file`/`--length-scale` в piper1-gpl против `--output_file`/`--length_scale` в старом rhasspy) **спрашивается у бинаря** через `piper --help`, а не угадывается. Модель голоса скачивается **только** по `--allow-download` (в `.vncache/piper-voices`, URL выводится из имени голоса); без флага — ошибка со ссылкой. Темп — множитель к нормальному темпу голоса, границы 0,5…2,0.

**Что получается на выходе:** мастер сразу `.opus` (тот же `encode_opus`: Opus 96k, loudnorm −19 LUFS, что и у боевых дублей) в `assets_src/voice/<lang>/<chNN>/<line_id>.opus`, строка в манифесте главы со `status: draft`. Синтез в WAV живёт только в стейджинге `.vncache/voice-tts/<ch>-<lang>` и стирается до и после прогона. Причина, почему мастер не WAV: `assets_src` — LFS-зона, и WAV-черновики раздували бы историю ради звука, который выбросят. Раскладку, сверку с ledger, атомарность и запись манифеста делает тот же `import_takes`, что и `vn voice import` — своего пути в `assets_src` у синтеза нет.

**Что синтезатор не произносит:** разметка Ren'Py снимается перед синтезом (`tts_text`) — теги `{w=0.5}`/`{i}` и интерполяции `[player_name]` иначе читались бы вслух буквально.

**Идемпотентность и границы:**

- по умолчанию берутся только реплики без покрытия плюс те, чей мастер пропал (манифест обещает дубль, файла нет — такую главу `vn voice validate` и так считает сломанной);
- `status: final` не трогается **никогда**, даже с `--regenerate-drafts`: перезаписать записанного актёра синтезом недопустимо;
- повтор без работы завершается успешно и **не резолвит бэкенд вообще** — на машине без TTS команда зелёная, если озвучивать нечего (на этом стоит и проверка в CI);
- при перегенерации прежний мастер в другом формате удаляется, иначе старый `.wav` заглушал бы новый `.opus`, а `validate` молчал;
- для `--lang`, отличного от исходного, текст берётся из PO: черновик дубляжа обязан говорить своим языком. Реплика без перевода даёт **warning**, а не молчаливую подмену исходным текстом.

**Как это видит релизный гейт:** ровно как любые черновые дубли — **драфты = WARN** (`release.py`), дыры покрытия в озвученных главах = FAIL. То есть TTS закрывает FAIL и оставляет WARN: «глава озвучена синтезом» видно в отчёте гейта до самого конца, пока дубли не заменят через `vn voice import`. Демо-глава `ch01` в репозитории — ровно такой случай: 14 реплик, все `draft`, поэтому `vn release validate` штатно даёт один WARN про черновую озвучку.

**Пер-персонажного TTS-профиля нет.** `ARCHITECTURE.md` в примере `character.yaml` показывает блок `voice.tts_draft: {engine, voice, rate}`, но схема `character@1` знает из голосового только `voice_tag` и стоит с `additionalProperties: false` — профиль живёт во флагах команды. Появится он — расширяются схема и `resolve_tts` (единственная точка), больше ничего.

**Проверено прогоном** (копия дерева, боевое не тронуто): бэкенд `say`, голос `Milena`; две дыры покрытия → покрытие `12/14 → 14/14`, `ffprobe` подтверждает валидный opus; повтор — ноль работы; `--char mira` — ровно её реплики; `--regenerate-drafts` пропускает единственный `final`-дубль и не трогает его мастер. **Живой путь `piper` на этой машине не проверялся** — бинаря нет; логика (оба диалекта флагов, разрешение модели, отказ без `--allow-download`) покрыта тестами на моках, e2e прогонялся на `say`.

**Решения по TTS на 2026** (из ресёрча; лицензии проверяйте сами по ссылкам перед коммерческим использованием). `piper` из таблицы ниже не выбирает: он даёт **черновик** для вычитки и сборки, а голоса персонажей — отдельное решение.
...
|---|---|---|---|
| `assets_total_mb` | **20000 МБ** (ADR-0012 поднял с 500) | `project.yaml: budgets` — источник истины; проверка — `budget_failures` (`../../tools/vn/src/vn/release.py:63-66`) | всё `game/assets/` целиком (спрайты + фоны + CG + видео + аудио) |
| `video_total_mb` | **8000 МБ** (ADR-0012 поднял с 300) | там же | только `game/assets/mov/` — то есть аудио конкурирует со статикой за остаток |
| **ADR-0004 (в редакции ADR-0012): бинари в `assets_src/` мимо LFS** | error на каждый файл мимо LFS + error на 50 МБ таких файлов суммарно; **warn-порога нет** | `lint.py:442-472` (секция 6a) | `.ogg` и wav-мастера озвучки покрыты `.gitattributes:26-27`, поэтому под порог не идут; файл с расширением вне правил LFS даст ошибку сразу |
