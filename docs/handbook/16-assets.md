...
| `shot_thumb` | **не один файл**, а дефолтный кадр послойного шота: `env` + первый вариант каждого слоя в z-порядке из `shots@1` | `shots/<chNN>/<sNNN>/<shot>.thumb.webp` — **рядом** с папкой слоёв (stem кончается на `.thumb`, поэтому обратно во входы шота не попадает) | alpha-over всех слоёв (`imaging.composite`, расхождение холста — ошибка) → миниатюра по `render.thumb`; в ключ кэша входят байты **всех** слоёв и их имена — смена z-порядка или дефолтного варианта пересобирает превью (G13) | `1` | `pipeline.py: _shot_thumb_jobs:275`, `_shot_preview_plans:577`, `imaging.composite:128` |
...
#    заодно assets build склеивает превью для галереи:
#    game/assets/shots/ch01/s030/sunset.thumb.webp (трансформация shot_thumb, §2)
...
# 4. В галерее (необязательно):  kind: shot, asset: shots/ch01/s030/sunset —
#    просмотрщик листает варианты слоёв живым layeredimage (15-gallery.md §…)
```
...
**Превью для галереи конвейер собирает сам** (как галерея это показывает и почему разблокировка шота считается по тегу+атрибуту — [15-gallery.md](15-gallery.md), «Послойные шоты в галерее»)**.** Плоского файла у шота не существует по замыслу, а сетке галереи нужна миниатюра — поэтому `vn assets build` читает `shots@1` (ровно ради этого) и склеивает дефолтный кадр: `env` + первый вариант каждого слоя в объявленном z-порядке → `game/assets/shots/<chNN>/<sNNN>/<shot>.thumb.webp` (трансформация `shot_thumb`, §2). Шот без декларации даёт **предупреждение**, а не ошибку: арт вперёд декларации — легитимное состояние работы. Точечная пересборка только превью после правки `order` или дефолтных вариантов отдельной командой не выведена — `vn assets build` инкрементален и пересоберёт их сам (ключ кэша включает имена и байты всех слоёв).
