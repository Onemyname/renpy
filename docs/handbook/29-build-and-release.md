...
**Снимок** (`cli.py`): после distribute `ci/rpyc-cache/<линия>/<version>/` очищается и туда копируются **все** `.rpyc` из-под `game/` — не только из `generated/`, потому что метки framework тоже попадают в сейвы и rollback.

**Кэш живёт в git** (`ci/rpyc-cache/`, негативное правило в `.gitignore` рядом с `rpyc-line`): с отключением CI (ADR-0020) кэш GitHub Actions перестал быть носителем линии имён, а копия в gitignored `build/` существовала бы на одной машине и умерла бы от первого `rm -rf build`. `build/dist/` по-прежнему снимок конкретной машины.
...
**Отдельная сущность, не путать: линия имён сейв-корпуса.** `ci/fixtures/rpyc-line/` — **52 `.rpyc`** в git (негативное правило в `.gitignore`, рядом с `ci/rpyc-cache/`). Ими управляют `_rpyc_line_restore` (`cli.py`) и `_rpyc_line_snapshot` (`cli.py`) вокруг `vn save corpus`, чтобы фикстуры в `ci/fixtures/saves/` (сейчас **две** — `schema1-demo.save` и `schema2-demo.save`) грузились детерминированно на любой машине. Подробности — [27-testing.md](27-testing.md).
...
| **Что мешает сегодня** | 1) в `project.yaml` стоят плейсхолдеры Valve (`appid: 480` — Spacewar, депоты 481–483, DLC 484–485 в манифестах паков): команда доходит до конца и пишет VDF, но steamcmd отвергнет заливку в чужое приложение. Это шесть **редактирований данных** (список — `docs/decisions/owner-decisions.md` §2.3), а не правки кода: раскладка депотов рабочая и понимает форматы всех трёх платформ (§14.2). 3) Дальше в силе остаются внешние блокеры: steam_api-редистрибутивов Valve на build-машине нет, аплоад запускает человек, ветку `beta` нужно создать в Steamworks заранее, и ни один прогон на живом Steam/Deck ещё не делался. Релизное следствие механики: **любая** ошибка staging = `_fail("steam: контент депотов не собран")` **до** записи VDF (`cli.py`) — промежуточных состояний не бывает |
...
| Что | Статус |
|---|---|
| Аплоад | **автоматизирован, но не проверен** — workflow `steam-upload` (`workflow_dispatch`, входы `flavor`/`branch`, §12) гоняет `release build` → `release steam` → `steamcmd`. Без секретов `STEAM_USERNAME`/`STEAM_CONFIG_VDF` шаг аплоада — no-op; с плейсхолдерным `appid: 480` дойдёт до steamcmd и будет отвергнут Valve. Живой выкладки не было ни разу |
...
- [ ] `vn release steam --flavor public` даёт VDF и раскладку без `error` — достижимо, но только на плейсхолдерных номерах и только когда собраны архивы всех трёх платформ (иначе `steam: контент депотов не собран`). Заливка в App ID 480 невозможна: приложение чужое (§14.1, `docs/decisions/owner-decisions.md` §2)
