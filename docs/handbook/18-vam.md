...
> **Статус подсистемы:** PARTIALLY IMPLEMENTED — «движок источника» готов и симметричен DAZ: схема `vam_render@1`, валидатор `vn assets vam validate`, provenance-шаг `vam_render`, детект установки в `vn pipeline doctor`, ветка в релизном гейте. **Но:** VaM на этой машине **не установлен** (doctor → WARN), деклараций в репозитории **ноль**, автоматизация захвата — **NOT IMPLEMENTED** (ни строки кода, ни планов в ADR-0006), а `docs/ARCHITECTURE.md` о VaM не знает вообще — 0 упоминаний.
...
**Что из этого важно нам практически:** VRRenderer отдаёт PNG-секвенцию, а наш видео-трек (`tools/vn/src/vn/assets/video.py`) ждёт **уже собранный контейнер** — `video_src/<group>/<name>.mp4|mov|mkv|webm|m4v|avi` (`../../tools/vn/src/vn/assets/pipeline.py:170-201`). Сборщика «секвенция → контейнер» в проекте **NOT IMPLEMENTED** (это же зафиксировано в аудите как GAP 11). Пока — руками через ffmpeg, который у нас и так обязателен:

```bash
# PNG-секвенция VaM -> mp4-сырец для нашего трека (руками, скрипта в проекте нет)
ffmpeg -framerate 30 -i shot_%06d.png -c:v libx264 -crf 12 -pix_fmt yuv420p \
       assets_src/video_src/ch01/hug.mp4
```
...
**Единственное место для воспроизводимости пакетов — `capture.plugins` и `capture.notes`.** Отдельного поля под пины `.var` в схеме нет. Пишите в `plugins` полную тройку `creator.package.version`, а не «Timeline»: ресёрч прямо предупреждает, что `scriptReferenceVersionOption: "Latest"` тихо меняет лицо персонажа между кадрами при обновлении зависимости. В `notes` — пресет PostMagic, множитель супер-семплинга, всё, без чего кадр не повторить.

### Известная дыра схемы: `.var` не объявишь

Паттерн `scene` принимает только `json | vac | vap`. Современный формат пакета VaM — **`.var`** — под паттерн **не попадает**. Практический обход: ссылайтесь на распакованную/сохранённую сцену `Saves/scene/<name>.json`, а сам `.var` пините в `capture.plugins`/`notes`. Системное решение — новая версия схемы `vam_render@2` (реестр схем версионируется, G16; см. [Кастомный движок](25-custom-engine.md)); статус: **NOT IMPLEMENTED**.

### Где лежат бинари сцены

`.json`/`.vac`/`.vap` (и тем более `.var`) — тяжёлые сырцы, их место в хранилище через `vn assets push`, а в git — только `<file>.manifest.json` (нормы G2/G21; docstring `../../tools/vn/src/vn/assets/vam.py:9-10`). **Честно:** хранилище `~/vn-assets-store` на этой машине **не существует**, `vn assets push` здесь ни разу не запускался, а бэкенд `type: s3` из `../../.vnstorage.yaml` осознанно бросает `StorageError` — NOT IMPLEMENTED. До развёртывания хранилища единственный легальный вариант — держать сцену вне репозитория и жить с тем, что валидатор будет ругаться (см. ниже). **Не коммитьте `.var` в git:** линт считает вес бинарей в `assets_src/` и краснеет после 50 МБ (ADR-0004, `../../tools/vn/src/vn/content/lint.py:371-399`), а один серьёзный `.var` перекрывает порог целиком.
...
**Улучшения тулинга, если решите вкладываться** (по приоритету, все — NOT IMPLEMENTED):

| Приоритет | Что | Где менять |
|---|---|---|
| P1 | Скаффолд `vn assets vam new <id>` — генерирует валидный шаблон декларации | `../../tools/vn/src/vn/cli.py` (группа `assets_vam`), рядом с `scaffold.py` |
| P1 | Сборщик «PNG-секвенция → `video_src`-контейнер» (нужен и VaM, и Sims 4) | `../../tools/vn/src/vn/assets/video.py` |
| P2 | `vam_render@2` с полем под пины `.var` и допуском `.var` в `scene` | `../../tools/schemas/` + `tools/vn/src/vn/assets/vam.py` |
| P2 | Симметрия гейта: `else` вместо `elif …checked` для VaM/Sims4 | `../../tools/vn/src/vn/release.py:358-378` |
| P3 | Сверка `capture.resolution` с реальным размером выхода | `tools/vn/src/vn/assets/vam.py` (и зеркально daz/sims4) |
| P3 | Строка про VaM/Sims4-декларации в `../conventions/naming.md` (там есть только DAZ, `naming.md:20`) | документация |
...
- **Не коммитить `.var`/`.vac`/сцены в `assets_src/`.** Порог ADR-0004 (warn > 30 МБ, error > 50 МБ нетекстовых байт) считает `vn content lint`, то есть покраснеет и CI, и релизный гейт.
...
- **Не рассчитывать на `.var` в поле `scene`** — паттерн схемы его не принимает (`json|vac|vap`), и `additionalProperties: false` не даст «дописать своё поле» рядом.
- **Не думать, что незахваченный выход поймает релиз.** Это WARN, не FAIL (`vam.py:68-70`), а при нуле деклараций гейт про VaM молчит совсем.
