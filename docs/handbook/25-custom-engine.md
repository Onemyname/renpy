...
| `release android build` | `--bundle`, `--install`, `--launch`, `--timeout` (def 3600) | `status` → `vn build` → штатная команда лаунчера `renpy.sh <SDK>/launcher android_build <проект> --destination …`. Вывод gradle/RAPT **не перехватывается**: молчащая сборка неотличима от зависшей | **IMPL** — APK собран и вскрыт 2026-08-18 (`dev.omniaid.vn-0.1.5-…-release.apk`, 52,1 МБ); `--bundle`/`--install`/`--launch` не прогонялись: нет Play-аккаунта и устройства, [39](39-platforms.md) §2.1.1 |
