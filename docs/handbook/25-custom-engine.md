...
| `release android status` | — | Готовность тулчейна: `rapt/`, совпадение хешей RAPT↔SDK, Android SDK (по `adb`), JDK 21, ключи подписи, `android.json`. Каждый недостающий пункт назван вместе с **командой**, которая его закрывает | **IMPL** (2026-08-18) — падает кодом 1; на этой машине печатает «OK — тулчейн готов» |
...
| **Зависимости** | Правка `cli.py` ломает CI (`.github/workflows/{ci,nightly,canary,release,steam-upload}.yml` вызывают команды `vn` по именам), хуки и любые скрипты. Переименование команды/флага — breaking change. Новая зависимость в `pyproject.toml` **обязана** получить пин в `tools/vn.lock`: лок ставится первым во всех пайплайнах, и незапиненный пакет приедет из PyPI произвольной версии. Новая джоба CI обязана ставить лок до editable и `ffmpeg` до `vn build` — оба инварианта стережёт `tools/vn/tests/test_ci_config.py` |
| **Валидация** | `python -m pytest tools/vn/tests -q` → 400 passed · `vn build --check` → `check: генерат свеж` · `vn doctor` → exit 0 · `vn --help` и `vn <группа> --help` без исключений |
