...
Шестой тип якоря — `beat` — схемой разрешён и **рабочий**, но вызов ставит автор: `vn.beat()` компилятор никогда не эмитит. Чтобы `unlock: {beat: x}` заработал, автор обязан сам поставить `$ vn.beat("x")` в теле `*.scene.rpy` (`030_flow.rpy`). Рабочий образец в дереве есть: вызов есть ровно один — `content/chapters/ch01_awakening/scenes/s030_rooftop.scene.rpy: ch01_s030__body`, и на нём держится единственная скрытая ачивка игры (`core.achievements.yaml: roof_alone`, `hidden: true`). Статус: **IMPLEMENTED** — рантайм готов, точка вызова ручная.
