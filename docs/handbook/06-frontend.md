...
| `main_menu()` | `core_screens.rpy:179` | главное меню | `tag menu`; wordmark `[config.name!t]`; «Продолжить» появляется по `renpy.newest_slot()` (:195). «Галерея» и «Достижения» есть и здесь — с **теми же** гейтами, что в рельсе (`vn_gal.categories()` / `vn_ach.visible_ids()`): игроку, который вернулся посмотреть открытое, незачем начинать игру, чтобы попасть в меню. Что гейты в двух местах не разъедутся — проверяет `test_platform.py` |
