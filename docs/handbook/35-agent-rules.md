...
> **Статус подсистемы:** NOT IMPLEMENTED (инфраструктура) — в репозитории **нет** ни `CLAUDE.md`, ни `AGENTS.md`, ни каталога `.claude/`, ни `REVIEW.md` (проверено `ls` на 2026-08-08: в корне только `README.md`, `CODEOWNERS`, `project.yaml`, `.vnstorage.yaml`, `.gitattributes`, `.gitignore`, `.gitlab-ci.yml`). Правила ниже существуют **только в этом файле хендбука**; §1 даёт готовый черновик, чтобы это исправить.
...
| `docs/adr/` | Точечные решения с обоснованием и планом отступления. ADR обязателен при изменении нормы раздела 0 | ЕСТЬ: 11 ADR + `template.md`; ADR-0008 — единственный со статусом «предложено» |
...
- Решения: `docs/adr/` (11 ADR; 0008 не принят).
...
| `pack_registry.owned()` | Всегда `True`: провайдера владения (Steam) нет |
