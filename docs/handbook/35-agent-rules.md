...
**Базовое правило: не уверен — прочитай файл.** Стоимость `Read` на порядок ниже стоимости неверного утверждения в отчёте, по которому владелец примет решение. Особенно это касается адресации в коде: номера строк в этом репозитории плывут при каждой правке, поэтому ссылайся ИМЕНЕМ — `cli.py: _check_budgets`, а не «cli.py, строка 156». Символьные ссылки проверяет тест `tools/vn/tests/test_docs_refs.py`; в вычищенных файлах (handbook 21/22) номера строк запрещены им же.
