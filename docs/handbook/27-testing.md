...
python -m pytest tools/vn/tests -q       # 2. 240 тестов
...
**23 файла `test_*.py` + `conftest.py`, 240 тестовых функций** (проверено `grep -h '^def test_' tools/vn/tests/*.py | wc -l` → 240). Все тесты лежат плоско в одной директории, тестовых классов нет.
...
| `test_scene_pipeline.py` | 23 | `vn.content.scenes`: контракт меток, запрет межсценовых jump, соответствие `return` ↔ `exits`, Variable Registry, эмиссия обвязки, фоны локаций | `_unit()`/`_analysis()` фабрикуют `SceneUnit` и результат парсера — SDK не нужен. e2e-компиляция демо-главы — `skipif` без `RENPY_SDK` (`:150`) |
...
| `test_compile.py` | 7 | `compile_content`: набор выходов пустого проекта, идемпотентность, точечная очистка осиротевших `.rpy`+`.rpyc`, `CompileError` при главах без `RENPY_SDK` | `BASE_OUTPUTS` — замороженный набор из 14 имён (`:11-26`); `monkeypatch.delenv("RENPY_SDK")` |
...
| `test_engine_compat.py` | 4 | Контракт-тесты G18 против пиннованного SDK: существование `renpy.call_stack_depth`/`get_return_stack`; равенство `VN_API_LEVEL` (тулинг) и `API_LEVEL` в `030_flow.rpy:9` | Маркер `requires_sdk` (`:11-14`). Тест про API_LEVEL SDK **не** требует — он регексом читает файл фреймворка |
...
- **Гейтинг по окружению.** Тихо скипаются на голой машине **17 тестов из 240**: `test_video.py` целиком (9, нужны `ffmpeg`+`ffprobe` в PATH), по одному ffmpeg-тесту в `test_voice.py:189` и `test_sources.py:193`, и 6 тестов под `RENPY_SDK` — три контракт-теста `test_engine_compat.py` (:25, :33, :55) и по одному e2e в `test_scene_pipeline.py:318`, `test_gallery.py:181`, `test_loc.py:448` (нужен `RENPY_SDK` с `renpy.py` внутри). Зелёный прогон без SDK и ffmpeg **не** означает, что путь через движок цел. Ни один из тестов, добавленных 2026-08-08 (`test_ci_config.py`, `test_crash_handler.py`, CLI-тесты `pack build`, ветка звука, схема манифеста, геометрия чипов), окружения не требует — они гоняются везде.
...
Каждый пункт — команда этого проекта. Ожидаемое «зелёное» состояние: `vn doctor` — 8 PASS, `pytest` — 240 passed, `vn release validate --flavor public` — ни одного FAIL.
...
- **Не считать зелёный `pytest` доказательством работоспособности игры.** 240 тестов почти не касаются `cli.py` (закрыт один `pack build`) и не исполняют `game/framework/**`. Без SDK и ffmpeg 17 из них ещё и скипнутся молча.
...
vn content compile --check
python -m pytest tools/vn/tests -q         # ожидание: 240 passed
...
| **Валидация** | `vn content lint` → `python -m pytest tools/vn/tests -q` (240) → `vn build --check` → `vn test smoke --picks 0,0` → `vn save check && vn save corpus` → `vn release validate --flavor public`. Для всего с 3-го пункта обязателен `RENPY_SDK` |
