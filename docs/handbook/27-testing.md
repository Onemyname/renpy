...
cd tools/vn && .venv/bin/python -m pytest -q && cd -   # 2. 457 passed (про cwd — §2!)
...
```bash
cd tools/vn && .venv/bin/python -m pytest -q                  # 457 passed
...
| Команда | Результат |
|---|---|
| `cd tools/vn && python -m pytest -q` | 457 passed |
| `PYTHONPATH=tools/vn python -m pytest tools/vn/tests -q` (из корня) | 457 passed |
...
| Окружение | Результат |
|---|---|
| SDK + ffmpeg | `457 passed` |
| без `RENPY_SDK`, ffmpeg есть | `447 passed, 10 skipped` |
| SDK есть, без ffmpeg | `444 passed, 12 skipped, **1 failed**` |
| без SDK и без ffmpeg | `434 passed, 22 skipped, **1 failed**` |
...
- **Не считать зелёный `pytest` доказательством работоспособности игры.** 457 тестов почти не касаются `cli.py` (покрыто ~7 из 69 команд) и не исполняют `game/framework/**`. Без SDK скипнутся 10, без ffmpeg — 12, и один при этом **упадёт** (§2.3).
...
(cd tools/vn && .venv/bin/python -m pytest -q)  # ожидание: 457 passed (§2.1!)
...
| **Валидация** | `vn content lint` → `(cd tools/vn && .venv/bin/python -m pytest -q)` (457) → `vn build --check` → `vn test oversample --scale 2` → `vn test smoke --picks 0,0` → `vn save check && vn save corpus` → `vn release validate --flavor public`. Для всего с 3-го пункта обязателен `RENPY_SDK` |
