...
4. **Экран.** Движок подхватывает наш `screen _exception` (`../../game/framework/20_ui/screens/crash_screen.rpy:19`). Игрок видит: заголовок, «Прогресс до последнего сохранения не пострадал», **путь к отчёту**, кнопки «Откатиться назад» / «Попробовать продолжить» / «Закрыть игру». Сырой трейсбек и кнопка Reload — **только при `config.developer`** (`:48-54,64`).
...
Что это даёт при разборе жалобы игрока: строка `build:` однозначно привязывает падение к сборке и флейвору (формат `<version>+<sha>.<flavor>.<YYYYmmddHHMM>`, `release.py:246`), breadcrumbs дают путь по меткам, которого в голом трейсбеке нет.

**Что просить у игрока:** файл из `crash/` — этого достаточно; `log.txt` бесполезен, если игра уже перезапускалась (перезаписывается).

**Строку `[vn] unhandled exception: …` в `log.txt` искать можно** — она пишется (`070_crash.rpy:50`). Раньше не писалась: `001_boot.rpy` ставил свой обработчик со старой трёхаргументной сигнатурой на `init -999`, а `070_crash.rpy` на `init -950` переприсваивал `config.exception_handler` — поле одно, побеждало второе, боотовый блок был мёртвым кодом. Теперь обработчик один, логирование живёт в нём, а в `001_boot.rpy:38-49` остался комментарий-указатель. Регрессия закрыта тестом: `tools/vn/tests/test_crash_handler.py` (2 теста) статически проверяет по `game/framework/**/*.rpy`, что присваивание `config.exception_handler` ровно одно и именно в `070_crash.rpy`, и что обработчик пишет строку в лог и возвращает `False`. Помните ограничение: `log.txt` перезаписывается на каждом старте, поэтому если игра уже перезапускалась — единственный источник это отчёт из `crash/`.
...
| `vn loc report` | «перевод отстал?» | покрытие и fuzzy по языкам. Флагов `--gate/--format` нет — гейтинг живёт только в релизном гейте (`release.py:408-434`) | PARTIAL |
...
```bash
VN_AUTOPILOT_SCREENS=gallery,preferences vn test smoke --picks 0,0
```
...
Важная деталь: `version.gen.rpy` содержит `define config.version = "0.1.4+dd1cb3e"` — git-sha **внутри генерата**. Любой коммит делает генерат формально устаревшим, поэтому `vn build --check` после `git commit` без пересборки — ожидаемо красный.
...
- **Не верить `errors.txt` в корне без проверки даты.** Лежащий сейчас файл — от `Sat Aug 8 12:59:41 2026` про `ypadding` у `hbox` в `history.rpy:42`; проблема давно исправлена (там теперь `spacing`).
...
- **Не удалять и не «чинить» блок автопилота** в `choice.rpy:53-54` и `core_screens.rpy:402-404` — прогон повиснет на меню до таймаута.
