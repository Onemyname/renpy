...
## 9а. Граф главы: карта, гайд и пересмотр сцен (ADR-0021) — IMPLEMENTED

Три фичи, которые игрок видит по-разному, — это **одна** структура данных. Из
деклараций главы (`exits`, `when`, `vars`) и AST авторских сцен компилятор
собирает граф и кладёт его в `game/generated/registry/flow.gen.rpy`
(`VN_FLOW`, схема `flow@1`). Рантайм-стор `vn_story`
(`00_core/100_story_graph.rpy`) отвечает по нему на вопросы UI:

| Фича | Экран / место | Что берёт из графа |
|---|---|---|
| Карта главы | `20_ui/screens/story_flow.rpy`, пункт «Карта главы» в навигации | узлы, рёбра, слои, концовки, кластеры, прогресс |
| Встроенный гайд | вторая строка под пунктом в `screens/choice.rpy` | `decisions` — какие пункты меню требует отмеченная цель |
| Пересмотр сцены | модалка узла карты | `preconds` — состояние входа для `renpy.call_replay` |

Что от вас требуется в декларациях главы — и ничего больше:

```yaml
# chapter.yaml — необязательные поля, нужные только карте
endings: [s050, s060]            # какие сцены считать финалами (иначе финал = последняя в scene_order)
clusters:                        # фазы главы: только заголовок и состав, БЕЗ координат
  - title_key: ch02.cluster.city
    scenes: [s010, s020, s030]
```

```yaml
# sNNN_*.scene.yaml
title_key: meta.scenes.ch02_s010.title   # заголовок узла; без него игрок увидит id «ch02_s010»
```

Руками **не** ведётся ничего из этого: списки несовместимых сцен, переменные для
входа в реплей, привязка подсказок к тексту пунктов, координаты узлов. Всё
выводится компилятором — именно поэтому карта не может разойтись с сюжетом.

Условие `when` разбирается в ограничения только внутри поддержанного
подмножества (`==`, `!=`, `<`, `<=`, `>`, `>=`, `and`, `or`, `not <переменная>`,
скобки, литералы). Всё остальное (`in`, вызовы, арифметика) — «непрозрачное»
условие: ребро остаётся проходимым, но предупреждение сборки скажет, что подсказок
и конфликтов по нему не будет. Держитесь подмножества — оно покрывает практику
ветвления, а гарантии стоят дороже красивого выражения.

Проверки:

```bash
vn content flow            # сцены, рёбра, меню, конфликтующие пары + ошибки прекондиций (гейт ci.yml)
vn content flow --json     # артефакт целиком — им сравнивают сборки диффом
vn test revisit            # движок реально проигрывает КАЖДОЕ состояние входа (гейт nightly.yml)
```

`vn content flow` краснеет, если сцена читает переменную, которой никто не пишет
и у которой нет `default`: тогда состояние входа в пересмотр не определить, и
игрок получил бы `NameError` вместо сцены.
