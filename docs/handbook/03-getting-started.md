...
| **Частые ошибки** | 1) не экспортировать `RENPY_SDK` в bash-вызове — `vn doctor` падает, `vn build` обманчиво зеленеет на кэше; 2) запускать pytest из корня репозитория (`No module named 'tests'`) или другим интерпретатором (`No module named 'yaml'`); 3) забыть `vn loc keys` после правки реплики — локально зелено, CI красный; 4) считать `docs/ARCHITECTURE.md` описанием построенного: `vn bootstrap --role` там упомянут, но в CLI его нет (exit 2); `vn validate`/`vn migrate`/`vn shell`/`vn test perf` из нормы убраны осознанно (ADR-0017, ADR-0019), а `vn build --use-artifact` наоборот реализован; 5) запускать `vn` из подкаталога вне репозитория — `find_root()` ищет `project.yaml` **и** `tools/schemas/`, `.git` не считается; 6) путать exit 3 (заглушка фазы) и exit 2 (команды нет вовсе) |
