...
- **Не ставить `vn content compile --check` в pre-commit hook** — git sha внутри `version.gen.rpy` сделает его красным после каждого коммита (§6).
