...
- `appid: null` — рабочее состояние (standalone). Сегодня там не `null`, а плейсхолдер Valve `480` (Spacewar) вместе с депотами 481–483: конвейер на них прогнан, но заливка в чужое приложение невозможна. Что заменить — `../decisions/owner-decisions.md` §2.3 (шесть чисел).
...
**Что делает `build`:** проверяет тулчейн **до** долгой сборки, затем зовёт штатную команду лаунчера `renpy.sh <SDK>/launcher android_build <проект> --destination …`. Лог gradle/RAPT идёт живьём — молчащая сборка неотличима от зависшей. Результат — `build/android/<package>-<version>-<numeric>-release.apk` (с `--bundle` — `.aab`); фактический прогон 2026-08-18 дал 52,1 МБ APK и 51,6 МБ бандла из 2,8 МБ `game/` за 52 с gradle. Вес пакета сразу сверяется с потолком канала: превышение делает прогон красным, но артефакт остаётся на диске.
