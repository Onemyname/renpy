...
> **Статус подсистемы:** справочник, а не подсистема. Каждая команда ниже проверена по `../../tools/vn/src/vn/cli.py` на текущем HEAD (`vn` 0.1.0, игра 0.1.5, Ren'Py 8.5.3.26051504): в CLI **20 групп/команд верхнего уровня и 64 листовых команды, из них 54 живых и 10 честных заглушек** (`exit 3`). Заглушки помечены в тексте отдельно — их вызов ничего не делает.
...
- `vn voice tts` — **заглушка** (`cli.py:1278-1281`, `exit 3`).
...
- Пытаться положить в галерею **шот** или спрайт: `asset` ограничен паттерном `^(cg|bg|mov)/…`, шоты и спрайты в схему не проходят вовсе. И показ шота не пишет `_seen_images` под именем CG-ассета — если кадр нужен в галерее, отдавайте его ещё и как отдельный `cg/…` с якорем `scene`.
...
- **Не путать два класса «нет команды».** `exit 3` («появится в фазе N») — честная заглушка: `vn migrate`, `vn shell`, `vn char new|validate|sheet`, `vn voice tts`, `vn save migrate`, `vn test replay|screens|paths`. `exit 2` — команды не существует вовсе.
...
- **Не доверять номерам строк в этом хендбуке буквально.** `cli.py` (1930 строк) и `release.py` правятся чаще всего, вставка блока в середину сдвигает сотню ссылок. Ищите по имени функции; номер — подсказка.
