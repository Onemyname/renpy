...
`vn.beat()` компилятор **никогда не эмитит** — это осознанно, — но вызов есть ровно один — `content/chapters/ch01_awakening/scenes/s030_rooftop.scene.rpy: ch01_s030__body`, и на нём держится единственная скрытая ачивка игры (`core.achievements.yaml: roof_alone`, `hidden: true`). Якорь `beat` в достижениях и галерее
...
| **G6** | :63 | `.rpyc` генерата — релизный артефакт: подкладывается перед компиляцией следующего релиза; очистка `generated/` точечная по диффу манифеста; полный wipe только в release-CI | IMPLEMENTED; кэш разложен по линиям флейворов — `ci/rpyc-cache/<флейвор>/<версия>/` (в git с 2026-08-21, ADR-0020), прямой `vn package` пишет в линию `dev` |
...
| **C15** | :175 | Фасад рантайма: `vn.checkpoint()`, `vn.beat()`, `vn.unwind_call_stack()`, `vn.check_scene_stack()`, глубина — `renpy.call_stack_depth()`; голых глобалов `vn_checkpoint`/`vn_beat` не существует | IMPLEMENTED; фасад с 2026-08 расширен пятью членами и объектом `pack_registry` из трёх других файлов на `init -998` (§ 4). `vn.beat()` компилятор **никогда не эмитит** (осознанно), но ручной вызов есть: вызов есть ровно один — `content/chapters/ch01_awakening/scenes/s030_rooftop.scene.rpy: ch01_s030__body`, и на нём держится единственная скрытая ачивка игры (`core.achievements.yaml: roof_alone`, `hidden: true`) — то есть член фасада НЕ мёртвый и удалению не подлежит (плюс бамп `API_LEVEL`, с которым сверяются манифесты паков). `API_LEVEL` остался 1: добавление члена совместимо назад |
