...
| **G19** | :91 | Перф-бюджеты в CI: cold start, baseline RSS (слабое железо + Android-эмулятор), суммарный размер `.rpyc`, размер реальных `.aab`/`.apk` по каналам | PARTIAL: только `cold_start_s` (в `vn test smoke`) и размеры каталогов; RSS, `.rpyc`, мобильные сборки — NOT IMPLEMENTED |
