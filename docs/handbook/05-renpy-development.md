...
| в контексте `label main_menu` overlay-экраны и таймеры не тикают | контекст главного меню отличается от игрового | автопилот поэтому и устроен так: `label main_menu` из `_AUTOPILOT_RPY` (`cli.py`) делает **один** вызов `vn_qa.autopilot_boot()` и сразу `return` — управление уходит в `label start`, и только там начинает тикать overlay-таймер `vn_autopilot` (`cli.py`) |
...
| Ren'Py 8.5 добавляет к имени слота токен: `1-1-LT1.save` | подпись сейва | корпус кладёт **оба** имени (`1-1-LT1.save` и `1-1.save`) в временный `--savedir`, движок подхватит известный ему (`cli.py` и ниже). Сейв, принесённый с чужой машины, движок встретит модальным подтверждением — для CI это решается собственным `--savedir`, а не переносом файлов в профиль игрока |
...
- **Переменные, которые должны сохраняться, не начинаются с `_`** — это конвенция проекта («`_` = транзиентная»), а не поведение движка: Ren'Py сохраняет и их. Поэтому `vn_menu` и `vn_scene` объявлены без префикса (`020_state.rpy:5-10`).
