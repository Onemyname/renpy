...
**Правило `_`-префикса.** Это конвенция проекта («переменная не является состоянием прохождения»), а НЕ поведение движка: Ren'Py сохраняет и `_`-переменные стора — `rollback.get_roots()` перебирает `ever_been_changed` без фильтра по имени. Отсюда:

* сохраняемые переменные **никогда** не начинаются с `_` (`../conventions/naming.md:29`);
* `snapshot()` явно пропускает все `_*`-атрибуты (`020_state.rpy:60`);
* служебные счётчики автопилота названы `_vn_ap_shot` / `_vn_ap_menu` — и движок их сериализует наравне с остальными (это и есть доказательство, что правила «`_` не сохраняется» нет); это видно по корпусному прогону, где нумерация скриншотов возобновилась с `shot003.png`;
...
`vn_state.snapshot()` (`game/framework/00_core/020_state.rpy:49-72`) превращает named stores в плоский `dict` с ключами `"store.var"`. Асимметрия чтения и записи сделана намеренно:
...
| `apply_snapshot(state)` | **только** объявленные пары из `SNAPSHOT_VARS`, и только те, что реально пришли в `state` | обратно в игру возвращается лишь то, что существует в текущей схеме — и лишь то, что миграция изменила (см. §5) |
...
**Фильтр по верхнему уровню недостаточен, поэтому есть второй — IMPLEMENTED 2026-08-22.** `_json_safe(value)` (`020_state.rpy:29-47`) пробует `json.dumps` на значении целиком и отбрасывает то, что раундтрип не переживёт. Причина: `_SIMPLE` смотрит только на сам объект, а `set` **внутри** объявленного списка проходит проверку типа и роняет `json.dumps` уже в `run_migrations` — то есть внутри `label after_load`, и загрузка старого сейва превращается в крэш-скрин вместо игры. Оракулом взят сам `json`: угадывать за него, какие вложенные типы сериализуемы, значит держать вторую реализацию `json`. Пропуск логируется отдельным сообщением («внутри значения тип, не переживающий json-раундтрип миграций»).

Значения на обратном пути прогоняются через `vn_compat.revertable()` (`020_state.rpy:89`, реализация — `game/framework/00_core/engine_compat/000_compat.rpy:17-30`): плоские `dict`/`list`/`set`, пришедшие из json и чистого python, не участвуют в rollback, пока не станут `RevertableDict`/`RevertableList`/`RevertableSet`.
...
project.yaml: save_schema: 2
        ...
        │
        └─> _collect_migrations (compile.py)  ->  правила: content/migrations.py
                 ...
                 │     (те же правила гоняет vn content lint — секция 6b lint.py,
                 │      иначе дыра проходила бы pre-push и падала только здесь)
                 └─> _emit_migrations (compile.py)
                          -> game/generated/state/migrations.gen.rpy  (init -960)
                                   ...
                                   грузится через exec(), кладётся в vn_state.MIGRATIONS
                                                │
                                       label after_load (020_state.rpy:150-178)
```
...
| работает над **dict-снапшотом**, а не над живыми переменными | перед цепочкой снапшот проходит json-раундтрип (`020_state.rpy:97`). Миграция видит только плоские типы; Revertable-обёртки движка не протекают в чистый python, и та же функция гоняется юнит-тестом без запуска игры (`tools/vn/tests/test_saves.py`) |
| обратно в сторы уезжает **только изменённое** — IMPLEMENTED 2026-08-22 | тот же раундтрип, который нужен миграциям, теряет часть формы: кортеж становится списком, ключи `dict` — строками. Раньше `apply_snapshot` получал ВЕСЬ снапшот, то есть переменная, которой не касалась ни одна миграция, молча возвращалась в игру после раундтрипа (`{1: …}` → `{"1": …}`, и следующий `d[1]` промахивался). Теперь `run_migrations` сравнивает канонические `json.dumps(…, sort_keys=True)` до и после цепочки и пишет только расхождения (`020_state.rpy:102,127-144`); сравнение по сериализации, а не по значениям, чтобы `True` и `1` не считались одинаковыми |
...
`020_state.rpy:150-178`. Весь control flow после загрузки живёт **только здесь**; `config.after_load_callbacks` зарезервирован под чистую валидацию без переходов (G5).
...
| разрыв в цепочке номеров | `vn_log("migration chain gap: N -> M — цепочка прервана")` и **обрыв** прогона на этой миграции (`020_state.rpy:107-115`) |
| `migrate` вернула не `dict` (типично — забытый `return`) | `vn_log("migration NNNN вернула <тип> вместо dict — цепочка прервана")` и такой же **обрыв**, как на дыре (`020_state.rpy:118-124`): следующая миграция получила бы `None`, а трейсбек у игрока хуже строки в логе |
| миграция положила не-json значение | значение всё равно уезжает в сторы — молча потерять результат миграции нельзя, — но факт нарушения контракта плоского состояния пишется в лог отдельной строкой с именем ключа (`020_state.rpy:135-141`) |
| догон триггеров | после миграций — `vn.recheck_triggers()`: ачивки и элементы галереи, привязанные к переменной, а не к якорю, проверяются на загрузке (иначе выставленное в хвосте сцены ждало бы следующего якоря следующей сессии) |

Почему схема поднимается только до фактически применённой: дыра в цепочке не должна пометить сейв «актуальным» без миграции.

**Обрыв на дыре — IMPLEMENTED 2026-08-22.** Раньше миграция за дырой всё равно исполнялась (лог был, прогон продолжался), и `vn_save_schema` перепрыгивал пропущенный номер: сейв помечался актуальным, не пройдя шаг, а повторная загрузка уже ничего не чинила. Миграция `N` написана под состояние **после** `N−1`, поэтому исполнять её поверх непройденной предыдущей нельзя. Непрерывность цепочки гарантируют компилятор и линтер (см. диаграмму выше), так что рантайм-ветка — защита в глубину: сюда доходит только генерат, собранный в обход сборки. Покрыто тестами `test_migration_chain_gap_breaks_the_chain` и `test_full_chain_applies_every_step` (`tools/vn/tests/test_saves.py`), которые исполняют настоящий блок `python in vn_state` без движка.
...
5. `vn build` — компилятор проверит резервирование и непрерывность цепочки и вошьёт исходник в `game/generated/state/migrations.gen.rpy`. Те же две проверки делает и `vn content lint` (то есть pre-push hook): правило живёт в одном месте — `tools/vn/src/vn/content/migrations.py`.
...
python -m pytest tools/vn/tests -q          # 373 теста; test_saves.py — эмиссия миграций и снапшота
