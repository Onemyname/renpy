...
| `is_desktop()` | `renpy.variant("pc")` — есть окно, мышь и право закрыть приложение | гейтит «Выйти» и группу «Окно / Полный экран» в `core_screens.rpy` (на мобильном окна нет, а приложение снимает система); что гейт не потеряется — проверяет `test_desktop_only_controls_are_gated` |
...
| **Android** | IMPLEMENTED как канал (2026-08-18 собраны и вскрыты оба пакета: APK 52,1 МБ и Play-бандл `.aab` 51,6 МБ; **на устройстве не запускался** — телефона нет, в Play не загружался — аккаунта нет) | Канал живёт в `vn release android {setup,status,preflight,build}` (`cli.py`, модуль `tools/vn/src/vn/android.py`): `setup {sdk,keys,config}` доводит тулчейн, дальше `build` вызывает **штатную** команду лаунчера `renpy.sh <SDK>/launcher android_build <проект> --destination …` (`launcher/game/android.rpy:739`). `vn package` про Android не знает намеренно: там `launcher distribute`, а мобильный канал — другая команда лаунчера, свой тулчейн и свои потолки. Оверсэмпл-варианты `@N` в мобильный пакет не едут (`options.rpy:92`, §10.3) | Профиль тача — токены: `gui.touch_min` 120 px на телефоне / 72 px на планшете, `gui.ui_scale = 1.4`, `gui.overscan_pad = 48` (`scale.rpy:33-39,53-64,79-83`). Копий экранов под мобильный нет | Неприменимо | Вёрстка эмулируется вариантами: `RENPY_VARIANT="touch small phone android mobile" vn test smoke` (проходит, 18 скриншотов). Сам пакет вскрыт: подписан (`META-INF/CERT.RSA`), без `@N`-вариантов, без dev-зоны `90_debug`, без `*.keystore`, без `.rpy` (только `.rpyc`). **На живом устройстве не запускался** — телефона нет |
...
|---|---|
| Прогон APK на живом устройстве | Пакет собран и вскрыт статически, но ни разу не запущен: Android-телефона нет. Проверять на устройстве надо ровно то, что статикой не видно — распаковка при первом запуске (пресплэш висит дольше всего), тач-зоны на палец, поведение при снятии приложения системой |
| Размер-бюджеты по каналам **в CI** | Локально факт по пакету сверяется с потолками (`package_facts`, вызывается из `build`), но джобы, которая собирает `.aab`/`.apk` на каждый релиз, нет: ~700 МБ Android SDK на прогон и ключ подписи в секретах — отдельное решение (`ARCHITECTURE.md` G19) |
| Провайдер владения для Google Play Billing | в `035_platform.rpy` нужна вторая ветка `init 999` (см. §9). Сейчас `android.json: store = none` — покупок в приложении нет вообще, и это осознанно: без витрины провайдер владения нечем питать |
...
| Оформление пакета — движковые дефолты | `android-icon_foreground.png`, `android-icon_background.png`, `android-presplash.jpg` в корне проекта отсутствуют, `preflight` предупреждает о каждом. Пресплэш виден дольше всего (первый запуск распаковывает файлы), и дефолтный узнаётся как «сделано на Ren'Py» |
...
```bash
vn release android setup sdk --download-rapt
                               # RAPT с renpy.org + Android SDK/NDK в пиннованный SDK
                               # ИНТЕРАКТИВНО: спросит про Android SDK Terms and Conditions
vn release android setup keys  # android.keystore + bundle.keystore в корне проекта
                               # ИНТЕРАКТИВНО: спросит имя для сертификата и бэкап
vn release android setup config
                               # android.json: пакет, ориентация, магазин, permissions
vn release android status      # RAPT, hash RAPT<->SDK, adb (Android SDK), JDK 21,
                               # android.keystore / bundle.keystore, android.json
                               # -> код 1 и команда, которой ставится каждый пробел
...
vn release android build [--bundle] [--install] [--launch] [--timeout 3600]
                               ...
                               # неотличима от зависшей; после сборки — ФАКТ по
                               # пакету против потолка канала (и крупнейшая
                               # запись внутри .aab против 500 МБ)
```

**Почему `setup` — это движковая команда, а не питон-скрипт.** Все три шага RAPT работают только ВНУТРИ процесса Ren'Py: установщик Android SDK импортирует `renpy.pygame` (нативный модуль), генератор ключей пишет резервную копию рядом с сейвами через `renpy.config`, `configure` читает `gradle.properties` из каталога `rapt/`. Поэтому `setup` запускает `renpy.sh <проект> vn_android_toolchain <rapt> <шаг>` — команду из dev-зоны `game/framework/90_debug/040_android_toolchain.rpy` (в дистрибутив не попадает), которая вызывает **те же функции RAPT, что и кнопки лаунчера**: `rapt.install_sdk.install_sdk`, `rapt.keys.generate_keys`, `rapt.configure.configure`. Своей реализации шагов у нас нет намеренно: на апдейте SDK меняется RAPT, а не наша обвязка. Что списки шагов CLI и движка не разъедутся — проверяет `test_engine_step_names_match_the_dev_command`.

Интерактивность шагов сохранена намеренно: шаг `sdk` требует принять Android SDK Terms and Conditions, шаг `keys` — подтвердить, что владелец сохранит копию ключа. Это решения человека, и автоматизировать их означало бы отвечать за него.

**Дефект RAPT 8.5.3, который приходится обходить:** консольный `rapt.interface.Interface.choice` печатает варианты как `write(текст, Style.BRIGHT)`, но `Style` в `rapt/interface.py` не импортирован — любой шаг с выбором падает `NameError`. Лаунчер этого не видит: его `MobileInterface` перекрывает `write`. Обход — подстановка отсутствующего имени в движковой команде, и только если RAPT сам его не завёл (в новой версии импорт может появиться).

Фактический прогон на этой машине (2026-08-18): все три шага `setup` прошли, `status` печатает «android status: OK — тулчейн готов», `build` собрал `build/android/dev.omniaid.vn-0.1.5-1787077172-release.apk` (54 619 408 Б ≈ 52,1 МБ) за 52 с gradle. Установлено: RAPT 8.5.3 (68 МБ архив → 233 МБ), Android SDK Platform 36 + platform-tools (итого `rapt/` 698 МБ), JDK на машине — 25 (требование RAPT — 21, проверка проходит). `android.json`: `package dev.omniaid.vn`, `orientation sensorLandscape`, `store none`, `permissions [VIBRATE, INTERNET]` — `INTERNET` добавляет сам RAPT, отказаться через `configure` нельзя. `preflight --bundle` проходит: `game/: 2.8 МБ, из них @N-вариантов 0.3 МБ (в мобильный пакет не едут) -> 153 МБ с накладными`, `кэш образов мобильного профиля: 200 МБ`, три предупреждения — нет `android-icon_foreground.png`, `android-icon_background.png`, `android-presplash.jpg`. Порядок в `build` намеренный: тулчейн проверяется **до** `vn build`, потому что сборка ассетов и компиляция идут минутами, а проверка — миллисекунды.

**Play-бандл собран тем же путём:** `--bundle` даёт `dev.omniaid.vn-0.1.5-<numeric>-release.aab`, 51,6 МБ, 382 записи; крупнейшая — `base/lib/x86_64/librenpython.so`, 38,9 МБ (в распакованном виде). Это ровно та величина, которую по `game/` увидеть нельзя: пофайловый лимит fast-follow пакета упирается не в наши ассеты, а в нативные библиотеки движка, — поэтому `package_facts` проверяет её по самому `.aab`, а `preflight` по `game/` (до сборки другого способа нет). В Play бандл не загружался: аккаунта нет, а `.aab` валидируется только в Play Console.

**Потолок канала сверяется с фактом, а не с оценкой.** После успешной сборки `vn release android build` печатает вес пакета и его долю потолка (`dev.omniaid.vn-0.1.5-…-release.apk: 52.1 МБ — 3% потолка universal APK (2048 МБ)`), а превышение делает прогон красным, оставляя артефакт на диске: понять, чем пакет раздут, можно только по файлу. Оценка `preflight` при этом остаётся входным гейтом — она отвечает на другой вопрос («стоит ли вообще запускать час gradle»).

**Что вскрытие APK показало (проверяемо `unzip -l`).** Пакет подписан (`META-INF/CERT.RSA`, `CERT.SF`, `MANIFEST.MF`); `@2`-вариантов — 0 (правило `build.classify("**@[2-9].*", "windows linux mac")` работает и на мобильном канале); dev-зоны `90_debug` — 0; `*.keystore` — 0; файлов `.rpy` — 0, только байткод (`x-cache/x-bytecode-312.rpyb`). То есть исключения из `options.rpy` проверены на настоящем мобильном пакете, а не только на десктопном.

**Дефект, который нашла первая же сборка: версия пакета теряла патч.** `versionName` берётся из `build.version`, а тот по умолчанию равен `config.version` — у нас это `0.1.5+<sha>` (sha нужен в крэш-репортах). RAPT выбрасывает из версии всё нечисловое (`rapt/build.py`: `[i for i in version.split(".") if i.isdigit()]`), поэтому первый APK вышел версией **0.1**, молча. Исправлено в эмиттере: `version.gen.rpy` задаёт `config.version` (диагностическая, с sha) и `build.version` (поставочная, чистый semver) раздельно; APK после правки — `0.1.5`. Проверяет `test_emitted_build_version_is_clean_semver`.

**Мобильный профиль памяти — отдельное число, и оно доезжает до движка.** `render.mobile.image_cache_mb` (`tools/schemas/project@1.schema.json`, дефолт `200`; в `project.yaml` не переопределён — десктопный `image_cache_mb: 1024`) применяется методом `RenderConfig.for_mobile()` (`assets/render_config.py`), и `preflight` считает на нём **ту же** модель памяти, что `vn assets memory`, но на масштабе `@1`: `@N`-варианты в мобильный пакет не едут, поэтому считать worst-case на `@2` было бы враньём в свою пользу. Отдельная проверка — «мобильный лимит ≥ десктопного» (предупреждение: почти всегда это забытая правка, а не решение). В **движок** лимит эмитится обеими ветками: `render.gen.rpy` содержит `define config.image_cache_size_mb = 1024` и присваивание `config.image_cache_size_mb = 200` под `if renpy.variant('mobile')` в `init python` того же приоритета (`init offset = -950`, то есть до `im.cache.init()` в `renpy/main.py:542`). Второй `define` на тот же `config` был бы переопределением, а не условием, — поэтому именно присваивание. Проверяет `test_mobile_cache_limit_reaches_the_engine`.
...
**Что осталось непроверенным честно:** APK ни разу не запускался на устройстве и не устанавливался (`--install`/`--launch` через adb — телефона нет); `.aab` собран, но в Play Console не загружался — аккаунта нет, а проверить бандл по-настоящему может только он; оценка накладных расходов пакета (150 МБ) теперь сверяема с фактом — `game/` 2,8 МБ дали APK 52,1 МБ, то есть накладные ≈49 МБ на трёх ABI, и 150 МБ остаются верхней границей, а не измерением; раскладка Android SDK внутри `rapt/` определяется по `rapt/sdk.txt` и `platform-tools/adb*` — если RAPT её сменит, диагностика даст ложное «нет adb» (сборку это не ломает).
...
**Экран достижений в игре появился** (`20_ui/screens/achievements.rpy`, пункт рельсы рядом с «Галереей»), поэтому в standalone игрок видит прогресс, а под Steam — ещё и в оверлее с профилем. Экран читает те же `visible()`/`has()`, поэтому ачивка невидимая для игрока не попадает ни на экран, ни в знаменатель счётчика, ни в Steam. Прогресс отдельной ачивки (`goal: {total, step}`), тост о выдаче и кнопка синхронизации в настройках — есть ([15-gallery.md](15-gallery.md)).

**Что мы у движка не используем.** Из трёх ключевых аргументов `achievement.register` (`00achievement.rpy:160-184`) используются два: `stat_max` и `stat_modulo` — их фасад подставляет для ачивок с блоком `goal`. Не используется `steam` (другое имя на стороне Steam): «API Name = наш id» — не ограничение движка, а **наше** решение, маппинг был бы вторым источником истины. Позиция тоста тоже не задаётся — движок ставит `POSITION_TOP_RIGHT` при инициализации (`00steam.rpy:1047`), а `achievement.steam_position` мы не трогаем. Полный перечень неиспользуемого — §10; как заводить ачивку в самом Steamworks — [40-steamworks.md](40-steamworks.md).
...
|---|---|---|---|
| **Steam Timeline** — фазы игры и режим в оверлее/клипах | `config.automatic_steam_timeline = True` по умолчанию (`00steam.rpy:28`); движок сам ставит game mode и читает `store.save_name` (`00steam.rpy:707-724`) | включено движком и наполнено нами: `vn.checkpoint` присваивает `save_name` = локализованное название главы (`030_flow.rpy`, `010_registry.rpy: chapter_title`) | IMPLEMENTED (на живом Steam не проверялось) |
| **Прогресс-ачивки и статы** | используем `stat_max`/`stat_modulo` в `register` и `achievement.progress` (`:248`) через провайдера в фасаде; НЕ используем `grant_progress` (`:276`), `get_progress` (`:233`), `indicate_achievement_progress` (`00steam.rpy:125`), `get/set_int_stat` (`:166`, `:183`), `retrieve_stats`/`store_stats` (`:43`, `:57`) | произвольные статы (не привязанные к ачивке) и ручное управление их синхронизацией | PARTIALLY IMPLEMENTED — прогресс ачивок есть (`goal`), отдельных статов нет |
