...
Классы без мастера считают геометрию сами и потому берут отсюда только набор
масштабов: видео — по heights (mov), 9-patch панели — умножением пиксельных
полей своей декларации на масштаб (ui, assets/ui.py). Апскейла у них нет:
источник не картинка, а параметры, поэтому крупный вариант просто рисуется.
"""
...
DEFAULTS: dict = {
    ...
    "classes": {
        ...
        "ui": {
            # 9-patch панели (ADR-0009). Мастера нет: геометрия целиком в
            # content/ui/panels.yaml, поэтому вариант @N — та же декларация,
            # нарисованная в N раз крупнее. Формат фиксирован (lossless WebP:
            # артефакты сжатия поехали бы по тянущимся краям), настраивать
            # тут нечего, кроме набора масштабов.
            "variants": [1, 2],
        },
    },
}
