...
from ..repo import load_yaml, unshipped_chapters, write_text_lf
...
def _units(root: Path) -> dict[str, list[tuple[str, str, str]]]:
    ...
    for ch_id, ledger in _ledgers(root).items():
        rows = []
        tail = " [QA-контент: игроку не уезжает, перевод не требуется]" \
            if ch_id in unshipped else ""
        for sid, say in sorted(ledger.get("says", {}).items()):
            who = say.get("who") or "рассказчик"
            rows.append((sid, say["text"], f"говорит: {who}; сцена {sid[:9]}{tail}"))
        for mid, menu in sorted(ledger.get("menus", {}).items()):
            for i, caption in enumerate(menu["items"]):
                rows.append((f"{mid}[{i}]", caption,
                             f"пункт меню {i}; сцена {mid[:9]}{tail}"))
        domains[ch_id] = rows
    common = []
...
def load_translations(root: Path, lang: str) -> dict[str, tuple[str, bool]]:
    """msgctxt -> (msgstr, fuzzy) по всем доменам языка (только переведённые).
    Публична намеренно: этим же входом пользуется voice-домен (черновики дубляжа
    говорят текстом из PO), а контракт между доменами не может быть приватным."""
...
def validate_translations(root: Path) -> list[str]:
    ...
    for lang in discover_languages(root):
        ...
        for ctx, (msgstr, fuzzy) in load_translations(root, lang.code).items():
...
def import_translations(root: Path) -> LocReport:
    ...
    for lang in langs:
        tr = load_translations(root, lang.code)
...
def report(root: Path) -> LocReport:
    rep = LocReport()
    # Покрытие считается по контенту, который РЕАЛЬНО уезжает игроку: глава пака
    # вне всех флейворов в знаменатель не входит (ADR-0021, unshipped_chapters).
    unshipped = unshipped_chapters(root)
    domains = {name: rows for name, rows in _units(root).items()
