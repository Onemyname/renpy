...
def lint(root: Path, layout: bool = True) -> LintReport:
    ...
    # Запись в renames.vars — ОБЯЗАТЕЛЬСТВО перенести значение, а не индульгенция.
    #
    ...
    # существует, поэтому по документированной процедуре (правка декларации +
    # правка ссылок + запись в renames.yaml) игрок терял прогресс МОЛЧА: старое
    # имя остаётся в сейве и больше никем не читается, новое получает дефолт
    # сборки. Проверено загрузкой фикстуры: `g.route` = 'prologue' под старым
    # именем, `g.path` = дефолт. Все ветвления по роуту уходят в пролог, а
    # галерейный анлок по этой переменной не срабатывает — открытый CG пропадает.
    #
    # Показательно, что для АССЕТОВ тот же класс закрыт: renames.assets реально
    # протаскивается в рантайм галереи (compile._asset_history), «иначе
    # переименование CG стирает игроку открытый кадр». Для переменных носителя
    # нет, поэтому единственный честный механизм — миграция.
    #
    # Требование дешёвое и проверяемое: для каждой пары `old: new` обязана
    # существовать миграция, упоминающая ОБА имени. Только старое было бы слишком
    # слабым условием — его случайно удовлетворяет любая давняя миграция, которая
    # трогала эту переменную по другому поводу (так и вышло на первом же прогоне:
    # `0002_route_prologue.py` упоминает `g.route`, и проверка «есть старое имя»
    # молча прошла). Перенос по определению ЧИТАЕТ старый ключ и ПИШЕТ новый,
    # поэтому оба имени в его исходнике присутствуют.
    if var_moves:
        mig_dir = root / "content" / "migrations"
        mig_sources: list[str] = []
        if mig_dir.is_dir():
            for f in sorted(mig_dir.glob("*.py")):
                mig_sources.append(f.read_text(encoding="utf-8", errors="replace"))

        def _mentions(src: str, ref: str) -> bool:
            return f'"{ref}"' in src or f"'{ref}'" in src

        for old_ref in sorted(var_moves):
            new_ref = (renames.get("vars") or {}).get(old_ref)
            if any(_mentions(s, old_ref) and (not new_ref or _mentions(s, new_ref))
                   for s in mig_sources):
                continue
            rep.error(
                f"content/renames.yaml: {old_ref} -> {new_ref} объявлено, но ни одна "
                f"миграция в content/migrations/ не переносит значение (в её "
                f"исходнике должны быть ОБА имени) — значение игрока останется под "
                f"старым ключом сейва, а новая переменная получит дефолт сборки, то "
                f"есть прогресс потеряется молча. Носителя переименования в рантайм "
                f"у переменных нет, в отличие от renames.assets: перенос делает "
                f"только миграция"
            )
