...
screen vn_flow_node(scene_id, xy, focus_default=False, continues=()):
    ...
    button:
        ...
        fixed:
            ...
            # Метки состояния — ОДНИМ столбиком в верхнем левом углу, а не
            # порознь по краям карточки. Порознь они и не помещались: подпись про
            # другую главу шириной в полкарточки, поставленная внизу справа,
            # налезала бы на заголовок сцены, который занимает всю нижнюю полосу.
            # Столбик решает и совпадение двух меток сразу: объявленная концовка
            # с выходом в другую главу — противоречие контента, но пока его не
            # ловит линт, карта обязана показать оба факта, а не спрятать один
            # под другим.
            vbox:
                spacing gui.sp_xs
                xpos gui.sp_s
                ypos gui.sp_xs
                # «Финал» — только у ПРОЙДЕННОГО узла. Метка стояла вне тумана
                # войны, и карта заранее говорила, какая из закрытых ветвей
                # кончается концовкой: в ch71 это 4 подписи на 7 ещё не открытых
                # узлов. Для скрытой концовки (ch73 s050 — секретная) это прямая
                # выдача того, что игрок должен найти. Структура развилки видна и
                # без метки — узел без исходящих линий и так лист графа.
                ...
                # ребро некуда, и раньше оно молча исчезало — узел читался
                # тупиком. Подпись, а не символ: стрелка в углу сообщает «что-то
                # есть», но не что именно, а карта существует ровно чтобы игрок
                # понимал структуру. Про туман войны здесь думать не нужно:
                # продолжение в другой главе — свойство ГРАФА, а не содержимого
                # сцены, и на карте он и так виден рёбрами.
                if scene_id in continues:
                    text vn_loc.t("ui.chart.continues"):
                        style "vn_flow_badge"
                        xsize VN_FLOW_NODE_W - 2 * gui.sp_s
            if _targeted:
                text "★" style "vn_flow_badge" xalign 1.0 ypos gui.sp_xs
...
# Метка состояния узла: только текст, без позиционирования. Позицию задаёт
# место — столбик меток слева сверху или инлайн у «★»: держать её в стиле значило
# бы, что стиль нельзя положить в контейнер.
style vn_flow_badge:
    font gui.interface_semibold_font
