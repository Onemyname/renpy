...
> **⚠️ НАСТРОЕНО НА ПЛЕЙСХОЛДЕРАХ (2026-08-21).** Конвейер прогнан целиком на
> тестовых номерах: App ID 480 (Spacewar), депоты 481–483, DLC 484–485 —
> `vn release steam` генерирует VDF и раскладывает депоты, preflight «0 шагов
> за владельцем». Перед реальной выкладкой: заменить ВСЕ номера на выданные
> Steamworks (project.yaml + packs/*/manifest.yaml), поставить steam_api-
> редистрибутив (лаунчер SDK → preferences → Install Steam Support) и включить
> workflow `steam-upload` (отключён вместе с CI, ADR-0020). Аплоад на чужой
> App ID 480 невозможен — steamcmd отвергнет без прав на приложение.
...
steam_api-библиотеки рядом с исполняемым файлом. Без неё (или при пустом
`platform.steam.appid` в project.yaml) игра — обычный standalone.

> **Статус: конвейер прогнан, заливка невозможна.** Приложения в Steamworks
> ещё нет; `platform.steam.appid` в `project.yaml` заполнен ПЛЕЙСХОЛДЕРОМ 480
> (публичный Spacewar Valve), депоты — 481–483, DLC — 484–485. На этих номерах
> проверено всё, что можно проверить без аккаунта: `vn release steam` генерирует
> VDF и раскладывает депоты, `vn release preflight` даёт «0 шагов за владельцем
> аккаунта». Не проверяемо без аккаунта ровно одно — сама заливка: steamcmd
> отвергнет запись в чужое приложение. Что заменить и в каком порядке —
> `docs/decisions/owner-decisions.md` §2; там же, чего это стоит вне репозитория.
> Всё ниже — процедура для человека, у которого аккаунт Steamworks есть; там, где
> утверждение непроверяемо, это сказано прямо.
